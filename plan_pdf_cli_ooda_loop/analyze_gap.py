...
def analyze_files():
    ...
    print(
        f"Gap: {len(gold_lines) - len(current_lines)} lines ({100 * (len(gold_lines) - len(current_lines)) / len(gold_lines):.1f}%)"
    )
    print()
    ...
    print(
        f"Gold single-char lines: {gold_single_char} ({100 * gold_single_char / len(gold_lines):.1f}%)"
    )
    print(
        f"Current single-char lines: {current_single_char} ({100 * current_single_char / len(current_lines):.1f}%)"
    )
    ...
    print(
        f"Current blank lines: {current_blank} ({100 * current_blank / len(current_lines):.1f}%)"
    )

    # Average line length
    gold_avg_len = sum(len(line.strip()) for line in gold_lines if line.strip()) / sum(
        1 for line in gold_lines if line.strip()
    )
    current_avg_len = sum(
        len(line.strip()) for line in current_lines if line.strip()
    ) / sum(1 for line in current_lines if line.strip())
    ...
    current_headers = [
        line.strip() for line in current_lines if line.strip().startswith("#")
    ]
    ...
    current_middle = current_lines[
        min(400, len(current_lines) - 100) : min(500, len(current_lines))
    ]
