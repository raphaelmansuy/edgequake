...
def run_synthetic_suite():
    ...
    pdfs = sorted(
        [p for p in test_data.glob("0*.pdf") if p.stem.split("_")[0].isdigit()]
    )
    ...
    for pdf in pdfs:
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                print(
                    f"✅ Success: {duration:.2f}s, {len(content)} chars, {result['tables']} tables"
                )
    ...
    print(
        f"✅ Successful: {success_count}/{len(results)} ({success_count/len(results)*100:.1f}%)"
    )
    ...
    if success_count > 0:
        ...
        avg_duration = (
            sum(r.get("duration_sec", 0) for r in results if r.get("success"))
            / success_count
        )
