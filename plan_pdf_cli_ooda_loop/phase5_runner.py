...
class Phase5Runner:
    def __init__(self):
        self.base_dir = Path("/Users/raphaelmansuy/Github/03-working/edgequake/edgequake")
        self.test_data_dir = self.base_dir / "crates/edgequake-pdf/test-data"
        self.binary = self.base_dir / "target/release/edgequake-pdf"
        self.output_dir = Path("/Users/raphaelmansuy/Github/03-working/edgequake/plan_pdf_cli_ooda_loop")
        self.results = []
    ...
    def run_extraction(self, pdf_path, output_path):
        ...
        try:
            result = subprocess.run(
                [str(self.binary), "convert", "-i", str(pdf_path), "-o", str(output_path)],
                capture_output=True,
                text=True,
                timeout=60
            )
            ...
            if result.returncode == 0:
                ...
                table_count = result.stdout.count("Lattice detected") + result.stdout.count("tables on page")
                ...
                output_lines = len(Path(output_path).read_text().split('\n')) if Path(output_path).exists() else 0
...
"""
    ...
    def execute_loop(self, loop_num, pdf_rel_path):
        ...
        if not pdf_path.exists():
            ...
            if result['success']:
                print(f"✅ ({result['duration']:.2f}s, {result['output_lines']} lines, {result.get('table_count', 0)} tables)")
            else:
...
def main():
    ...
    print(f"   Success: {len([r for r in runner.results if r['success']])}/{len(runner.results)}")
