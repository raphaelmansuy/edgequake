...
class OODALoopRunner:
    ...
    def print_summary(self):
        ...
        success_count = sum(1 for r in self.results if r["stages"]["observe"]["success"])
        
        print(f"✅ Successful extractions: {success_count}/{len(self.results)}")
        print(f"❌ Failed extractions: {len(self.results) - success_count}/{len(self.results)}")
        
        if success_count > 0:
            scores = [r["stages"]["orient"]["score"] for r in self.results if r["stages"]["observe"]["success"]]
            avg_score = sum(scores) / len(scores)
            print(f"📊 Average quality score: {avg_score:.1f}/100")
            
            # Table stats
            table_counts = [r["stages"]["observe"]["table_count"] for r in self.results if r["stages"]["observe"]["success"]]
            total_tables = sum(table_counts)
            print(f"📋 Total tables extracted: {total_tables}")
            
            # Timing stats
            durations = [r["stages"]["observe"]["duration_sec"] for r in self.results if r["stages"]["observe"]["success"]]
            avg_duration = sum(durations) / len(durations)
            print(f"⏱️  Average extraction time: {avg_duration:.2f}s")


def main():
    base_dir = Path("/Users/raphaelmansuy/Github/03-working/edgequake/plan_pdf_cli_ooda_loop")
    runner = OODALoopRunner(base_dir)
    
    # Phase 1: Real academic papers
    real_dataset = Path("/Users/raphaelmansuy/Github/03-working/edgequake/edgequake/crates/edgequake-pdf/test-data/real_dataset")
