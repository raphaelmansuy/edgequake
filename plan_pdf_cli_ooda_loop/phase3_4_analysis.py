...
def analyze_conversion_quality(
    pdf_path: Path, output_md: Path, gold_md: Path = None
) -> Dict:
    ...
    analysis["quality_grade"] = (
        "A"
        if score >= 90
        else "B" if score >= 75 else "C" if score >= 60 else "D" if score >= 50 else "F"
    )
...
def run_comprehensive_analysis():
    ...
    phase2_results = json.loads(
        (plan_dir / "phase2_synthetic_results.json").read_text()
    )
    ...
    for result in phase1_results[:3]:  # First 3 papers
        ...
        output_path = (
            plan_dir / result["stages"]["observe"]["output_path"].split("/")[-1]
        )
        ...
        if output_path.exists():
            ...
            print(
                f"  Quality Score: {analysis['quality_score']}/100 (Grade: {analysis['quality_grade']})"
            )
            print(f"  Tables: {analysis['structure_metrics']['tables']}")
            print(
                f"  Headings: {analysis['structure_metrics']['h1_headings'] + analysis['structure_metrics']['h2_headings']}"
            )
            if "gold_comparison" in analysis:
                print(
                    f"  Gold Similarity: {analysis['gold_comparison']['similarity_pct']:.1f}%"
                )
    ...
    for pdf_name, note in edge_cases:
        ...
        try:
            ...
            if proc.returncode == 0 and output_path.exists():
                ...
                print(
                    f"  ✅ Extracted ({analysis['basic_metrics']['char_count']} chars)"
                )
            else:
    ...
    for run in range(3):
        ...
        print(
            f"  Time: {duration:.2f}s, Size: {len(content)} chars, Tables: {content.count('|---|')}"
        )
    ...
    print(
        f"    Avg duration: {sum(t['duration_sec'] for t in consistency_tests) / 3:.2f}s"
    )
    ...
    comprehensive_results = {
        ...
        "consistency_validation": {
            ...
            "avg_duration_sec": round(
                sum(t["duration_sec"] for t in consistency_tests) / 3, 2
            ),
        },
    }
