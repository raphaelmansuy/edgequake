...
async fn dispatch_migrate(rest: &[String]) -> Result<()> {
    match rest.first().map(String::as_str) {
        ...
        None => migrate_apply::run_migrate_cli(rest).await,
...
async fn async_main_postgres() -> Result<()> {
    ...
    // SPEC-150 WP-5: when SCHEMA_GATE=wait, bind a lite /live+/ready router and
    // poll the ledger until Serve (or exit 78 on Refuse).
    {
        ...
        let bind_addr: std::net::SocketAddr = format!("{host}:{port}")
            .parse()
            .context("invalid HOST:PORT for schema gate wait")?;
        schema_gate_wait::wait_until_schema_ready(&database_url, bind_addr).await?;
    }
