    ...
    async fn setup_extensions(&self, pool: &PgPool) -> Result<()> {
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                tracing::warn!("Apache AGE extension not available: {}. Graph operations will use fallback.", e);
            }
        }
...
impl std::fmt::Debug for PostgresPool {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PostgresPool")
            ...
            .field("connected", &self.pool.try_read().map(|g| g.is_some()).unwrap_or(false))
            .finish()
    }
}
