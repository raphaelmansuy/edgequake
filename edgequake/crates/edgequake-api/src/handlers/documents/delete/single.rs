...
use super::super::storage_helpers::get_workspace_vector_storage_for_delete;
...
pub async fn delete_document(
    ...
    // MISSION-02-GAP-1: Delegate graph cleanup to the shared helper instead of
    // duplicating ~200 lines of node/edge/orphan processing logic here.
    //
    // MISSION-02-GAP-2: Pass workspace_id so the helper can skip nodes that belong
    // to other workspaces (isolation guarantee).
    //
    // MISSION-02-GAP-3: The helper now only makes 2 graph calls (nodes + edges) by
    // tracking deleted nodes locally for orphan detection.
    //
    // source_prefixes covers both the KV key prefix and the JSON id in the historical
    // key/id mismatch case so nodes/edges referencing either form are cleaned up.
    let source_prefix_strs: Vec<&str> = source_prefixes.iter().map(|s| s.as_str()).collect();
    let graph_stats = super::super::storage_helpers::cleanup_document_graph_data(
        &source_prefix_strs,
        Some(workspace_id_for_storage.as_str()),
        &state.graph_storage,
