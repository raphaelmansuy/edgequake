...
async fn test_tenant_isolation_e2e() {
    ...
    // Register tenants
    tenant_service.add_tenant(TenantConfig::new("tenant_a")).await;
    tenant_service.add_tenant(TenantConfig::new("tenant_b")).await;
    ...
    let instance_a = manager.get_instance("tenant_a", "default", Some("user_1")).await.unwrap();
    {
        let mut rag = instance_a.write().await;
        
        // Use shared storage
        rag.set_storage_backends(shared_kv.clone(), shared_vector.clone(), shared_graph.clone());
        rag.set_providers(mock_llm.clone(), mock_llm.clone());
        
        rag.initialize().await.expect("Failed to initialize RAG A");
        
        rag.insert("This is a secret document for Project Alpha belonging to Tenant A.", Some("doc_a"))
            .await
            .expect("Failed to upload doc A");
    }

    // 3. Tenant B uploads a different document
    let instance_b = manager.get_instance("tenant_b", "default", Some("user_1")).await.unwrap();
    {
        ...
        rag.insert("This is a secret document for Project Beta belonging to Tenant B.", Some("doc_b"))
            .await
    ...
    // 4. Verify Isolation: Tenant A queries for its content
    {
        ...
        let result = rag.query("What is Project Alpha?", Some(params)).await.unwrap();
        
        let entities: Vec<String> = result.context.entities.iter().map(|e| e.name.clone()).collect();
        println!("Tenant A retrieved entities: {:?}", entities);
        assert!(entities.contains(&"PROJECTALPHA".to_string()), "Tenant A should see its own project");
        assert!(!entities.contains(&"PROJECTBETA".to_string()), "Tenant A should NOT see Tenant B's project");

        // Verify chunk isolation
        let chunks: Vec<String> = result.context.chunks.iter().map(|c| c.content.clone()).collect();
        println!("Tenant A retrieved chunks: {:?}", chunks);
        assert!(chunks.iter().any(|c| c.contains("Project Alpha")), "Tenant A should see its own chunks");
        assert!(!chunks.iter().any(|c| c.contains("Project Beta")), "Tenant A should NOT see Tenant B's chunks");
    }
    ...
    // 5. Verify Isolation: Tenant B queries for its content
    {
        ...
        let result = rag.query("What is Project Beta?", Some(params)).await.unwrap();
        
        let entities: Vec<String> = result.context.entities.iter().map(|e| e.name.clone()).collect();
        println!("Tenant B retrieved entities: {:?}", entities);
        assert!(entities.contains(&"PROJECTBETA".to_string()), "Tenant B should see its own project");
        assert!(!entities.contains(&"PROJECTALPHA".to_string()), "Tenant B should NOT see Tenant A's project");

        // Verify chunk isolation
        let chunks: Vec<String> = result.context.chunks.iter().map(|c| c.content.clone()).collect();
        println!("Tenant B retrieved chunks: {:?}", chunks);
        assert!(chunks.iter().any(|c| c.contains("Project Beta")), "Tenant B should see its own chunks");
        assert!(!chunks.iter().any(|c| c.contains("Project Alpha")), "Tenant B should NOT see Tenant A's chunks");
    }

    // 6. Verify Security: User without access to Tenant C
    tenant_service.add_tenant(TenantConfig::new("tenant_c")).await;
    let result = manager.get_instance("tenant_c", "default", Some("user_1")).await;
