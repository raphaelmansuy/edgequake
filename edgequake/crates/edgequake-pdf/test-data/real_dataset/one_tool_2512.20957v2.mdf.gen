...
repository-level retrieval task and rely on multiple auxiliary tools, which overlook code execution logic and complicate model control. We propose Repo Navigator, an LLM agent equipped with single execution-aware tool-jumping to the 
...
model surpassing 32B competitors, and even the 32B model exceeding closed-source models such 
...
pabilities (Shen, 2024; Yuan et al., 2024; Lu et al., 2024). In the domain of software engineering (SWE), although LLM agents can effectively handle simple programming
...
sues. All pretrained LLMs can not process the whole reposrectly to pretrained LLMs (Liu et al., 2023; Chen et al., 2025;

(Jimenez et al., 2023) provides moderate gains, it remains

far from enabling robust repository-level reasoning.

Most existing agents rely on test-time scaling applied diin a single forward pass. Agents must therefore iteratively
...
However, mainstream LLMs are rarely exposed to such agentic interaction patterns during pretraining and typically acquire tool usage only through few-shot prompting. Such 
...
in domains including retrieval agents (Jin et al., 2025), GUI agents (Hong et al., 2024), and math agents (Yan et al., 2025).
...
solvability. Crucially, localization outputs a discrete set of paths, enabling verifiable, string-level evaluation that is when the tools are search engines (Jin et al., 2025), python compatible with scalable training frameworks such as SFT and RLVR.
...
dencies, we focus on enhancing their ability tojump across the repository-that is, to follow and inspect the source def-spository as a graph and applied graph-level searching tools

we introduce a single, structurally grounded tool: jump which retrieves the precise definition of a given symbol.

Details of this tool are provided in Sec. 3.3.

Our main contributions are threefold: (1) We propose the first repo-level localization agent trained on reinforcement learning directly from the pretrained model, regardless of distillation from a close-source model. (2) We design a repository-navigation agent that operates by performing 

mantics. (3) We demonstrate that one unified tool signififormat or failed parameter parsing. Thus, training a LLM

multi-tool pipelines.

to master new-defined tool is critical for LLM agents. Intrain a student model via supervised finetuning (SFT) (Chen

tuitively, the tool-calling trajectories can be generated by a more powerful LLM, and such trajectories can be used to
...
with tools for interacting with computing environments. Workflow-based methods such as Agentless (Xia et al.,

 2024) decompose issue resolution into localization, repair,

and validation subproblems. Chen et al. (2025) builds the reet al., 2025b) equip conceptual information of the respository to enable repo-level understanding. These pipelines

for localization, and Wang et al. (2025a) furthermore intement learning and achieve strong performance. However,
...
Repo Navigator uses a single-tool design to avoid multitool orchestration overhead. At each step the policyπθ decides whether to continue reasoning or to emit a JSONfile are parsed to the tool. The agent receives structured ob-

formatted tool call, while a symbol and its corresponding

servations (code snippets or error messages), then continues reasoning until termination. The loop is reason *→act →* observe.
...
We present Repo Navigator, a reinforcement-learning agent
 ...
 definition of any symbols in a given file, (2) a reasoning- action agent loop that alternates between natural-language reasoning and tool invocation, and (3) a GRPO-based RL algorithm for optimizing long-horizon tool-augmented tra-jectories. Below we provide the formal problem setting and
...
same code snippet, we additionally parse an indexto the tool, which allows for accurate resolution of *ℓ*.

Syntactic Analysis In this process, the source file is parsed into an abstract syntax tree (AST). The syntactic role of s (e.g., name, attribute access, or call expression)where the first term is the standard policy gradient objective determines the subsequent resolution strategy. For attributewith an estimated advantage function A

expressions a.b , Pyright treatsa as a receiver expressionactions that lead to higher-than-expected returns. The secwhose type must be inferred prior to member lookup.ond term is a Kullback-Leibler (KL) divergence penalty,

scaled by a coefficientβ, which acts as a trust region, predate definitions are searched along a scope chain the previous policy π

Lexical Scope Resolution For a name symbol *x , candi-* venting the updated policy π
...
where MRO(*t)* denotes the method resolution order of typeis incorrect, or the symbol parsed does not exist, or for any
...
Given history h= (*q, o, a*), the agent samplessubset of SWE-bench. We additionally test our method on either a natural-language reasoning step r ∼ *π (·|h*) or a a subset of SWE-bench-pro (Yang et al., 2025b) (which structured tool calla ∼ *π ( ·|h*). Tool calls must satisfyis a new and more difficult benchmark) for generalization. a JSON grammar enforced via constrained decoding. The loop continues until the agent outputs its final localizationgolden patches. All datasets are open-source and are built *Y .*ˆ on real-world github issues.
...
where each pair ( f , p ) denotes a file path and a source*i i* Reference Policy Optimization (GRPO), which has the loss

position corresponding to a valid definition site of s . Infunction: practice, we use filepath and symbol to resolve ℓ. If
...
to train the agent directly from the pretrained model, withare sets of strings, recall and precision singularly can not no teacher model required. In practice, we apply Groupreflect the performance fairly. Thus, we utilize Sample-F1
...
Qwen2.5-7B-Instruct with Repo Navigator to sample each
...
our base model. Bold numbers denote the best performance among same-size models; underline numbers denote the best trainingfree performance among same-size models;yellow backgroundillustrates training-free Repo Navigator; blue backgroundillustrates

train it with 16 Tesla-A100-80G GPUs. We apply verl4.2. Effectiveness
...
| Agentless Training Free 11.08 7.31 7.98 5.80 43.07 13.89 20.07 | 13.11 |  |  |  |  |  |  |  |  |  |
| Repo Searcher Training Free 2.00 1.29 1.45 1.00 13.51 3.43 5.31 | 3.24 |  |  |  |  |  |  |  |  |  |
| Repo Navigator Training Free 13.96 | 20.25 |  |  |  |  |  |  |  |  |  |
| Repo Navigator GRPO 18.13 | 29.44 |  |  |  |  |  |  |  |  |  |
| 13.15 18.89 | 12.35 |  |  |  |  |  |  |  |  |  |
| 10.67 9.20 30.50 37.24 31.86 | 28.82 |  |  |  |  |  |  |  |  |  |
| 14.29 12.02 36.36 48.13 39.74 | 36.36 |  |  |  |  |  |  |  |  |  |

numbers denote the best performance among same-size models; underline numbers denote the best training-free performance among same-size models; yellow backgroundillustrates training-free Repo Navigator;blue backgroundillustrates Repo Navigator trained with
...
ization, our method surpasses all baseline methods with the same model size. Moreover, if we train Repo Navigator

with GRPO, our 7B model surpasses 14B baselines, and our

contributes to the validness of Repo Navigator furthermore.

cantly lower precision score than Repo Navigator, and result in lower S-F1 and IoU. This indicates that Repo Navigator
...
reported the results in Fig. 4.2. As shown in the figure, allowing more tool-calling turns consistently leads to improved performance for Repo Navigator, both before and after reresults empirically validate the scaling law of tool-calling
...
has the highest performance on issue resolution, while reinhybrid reward (with tool-calling success rate) has highersince additional tools often introduce new and unfamiliar
...
7
 ...
 method outperforms Repo Searcher for 14B models. This is probably due to the simplified tool we integrate to the agent in this context.

(see Sec. 5 for more details).

To assess the generalizability of Repo Navigator, we present

its performance on Python samples from the SWE-benchon this dataset are consistent with those observed on SWEbench Verified. While we cannot fully exclude the potential

Pro dataset (Yang et al., 2025b) in Table 2. The results

influence of data leakage in SWE-bench Verified, we can make a stronger claim regarding SWE-bench Pro, as it was released after the publication of the Qwen2.5 series.

### 4.3. Training Strategy Comparison

To explore the capability of GRPO on agentic training, we

compare GRPO against RFT-only and RFT+GRPO. As presented in Fig. 3, directly training with GRPO outperformes RFT-only and RFT+GRPO. Moreover, although RFT has accetable performance, the more steps RFT proceeds, the less improvement GRPO makes after the cold start. This conclusion contradicts with previous SWE agents trained with RL

(Ma et al., 2025), however, it aligns with the broader field of

reinforcement learning, where RFT and SFT (as a cold start) is effective only when the pretrained model is not strong enough (Guo et al., 2024a). When the pretrained model is strong enough and data is high-quality, directly training a model with RL is better than training after SFT (RFT) as its cold start.
...
In this section, we analyze the logic behind Repo Navigasembled functions is more effective than building multiple
...
When only a single tool-specifically thejump tool-is reto what this tool can access. In this case, the set of possible
...
Jiang et al., 2025) and present the result in Table. 4. Getoutputs the class/function definition. Get Struc takes no input and outputs the repository's structure. The results clearly
...
### 5.2. Impact on Tool-Calling Success Rate
 ...
 ity of thei-th call be*p . For a task that requires**i* *k sequential* tool invocations, the overall success rate can be expressed
...
within a repository. For ajumptool that navigates to symbol definitions, its access scope can be obtained by starting reducing the complexity and brittleness of multi-step tool
...
Intersection over Union (IoU) between the prediction set and the groundtruth set, using the jumptool results in a 
...
Our findings highlight the importance of aligning agent tool- References ing with real execution structure, and show that efficient reasoning-tool co-training can unlock substantial gains even for medium-sized open-source models. Future work will 
...
language model for gui agents. In Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition, pp. 14281-14290, 2024.
...
C. H., Gonzalez, J. E., Zhang, H., and Stoica, I. Efficient memory management for large language model serving

with pagedattention. In Proceedings of the ACM SIGOPS 29th Symposium on Operating Systems Principles, 2023.

Langley, P. Crafting papers on machine learning. In Langley,

P. (ed.), on Machine Learning (ICML 2000), pp. 1207-1216, Stan-ford, CA, 2000. Morgan Kaufmann.

Li, Y., Wen, H., Wang, W., Li, X., Yuan, Y., Liu, G., Liu,

J., Xu, W., Wang, X., Sun, Y., et al. Personal llm agents:

Insights and survey about the capability, efficiency and security. arXiv preprint arXiv:2401.05459, 2024.
...
F., Ma, S., Ma, S., Li, M., Yin, G., et al. Toolsand-box: A stateful, conversational, interactive evaluation benchmark for llm tool use capabilities. arXiv preprint arXiv:2408.04682, 2024.
...
B., Roongta, M., Zhang, C., Li, L. E., Popa, R. A.,
 ...
 of-the-art coding agent from scratch by scaling rl. https://pretty-radio-b75.notion.site/ DeepSWE-Training-a-Fully-Open-sourced-State-of-the-Art-Coding-Agent-by-Scaling-RL-22281902c1468193aabbe9a8c59bbe33
...
nighoff, N., Zhang, Y., Hui, B., Lin, J., Brennan, R., Peng, H., Ji, H., and Neubig, G. Openhands: An
...
Xia, C. S., Deng, Y., Dunn, S., and Zhang, L. Agentless: Depreprint arXiv:2407.01489, 2024.

mystifying llm-based software engineering agents. arXiv

Yan, Y., Wang, S., Huo, J., Yu, P. S., Hu, X., and Wen, Q.

Mathagent: Leveraging a mixture-of-math-agent framework for real-world multimodal mathematical error de-
...
Narasimhan, K. R., et al. Swe-bench multimodal: Do ai systems generalize to visual software domains? arXiv, preprint arXiv:2410.03859, 2024b.

One Tool Is Enough: Reinforcement Learning for Repository-Level LLM Agents

Jiang, Z., Ren, X., Yan, M., Jiang, W., Li, Y., and Schmidgall, S., Su, Y., Wang, Z., Sun, X., Wu, J., Yu, X.,

Liu, Z. Cosil: Software issue localization via llm-Liu, J., Moor, M., Liu, Z., and Barsoum, E. Agent labdriven code repository graph searching. arXiv preprint oratory: Using llm agents as research assistants. arXiv
...
K., and Zhao, J. Orcaloca: An llm agent frameworkbased on the classes and functions. for software issue localization, 2025b. URLhttps:
...
reliable reinforcement learning for advanced reasoningautomatic LLM agent besides its planning prompt concategeneous graph, whose nodes are files, classes, and functions.

tasks. arXiv preprint arXiv:2504.05118, 2025. nated into the context at the beginning of the searching

 are equipped to the LLM for multi-hop reasoning.

 introduced the first training framework calization agents, which is composed of distilling from a close-source model (Claude3.7-Sonnet in Repo Seacher) as
...
Tool Train for lollm reinforcement learning system at scale. arXiv preprint Agentless (Xia et al., 2024) is a workflow for

Ours Compared with all baselines, we are the first fullyissue localization. First, it identifies suspicious files in the
...
tool is already powerful enough
...
In practice, when the prediction set*Y is empty (for instance,*ˆ total failure), we set recall, precision, sample-F1, and IoU to zero. We use the function-level localization result of different methods and apply the patch generation backend 
...
is generated, the agent loop will stop automatically. The aforementioned process is an automatic agentic framework,
...
this concern by results in Tabel. 2. The SWE-bench dataset was published in 2025, while the Qwen2.5 series

were published in 2024. Moreover, we exclude the samples

in the training dataset if the repository also appears in SWE- (Shen, 2024) and present an example. Noted, we do not bench Verified or SWE-bench Pro.

Groundtruth Retrieval A limitation of our work lies in the extraction of groundtruth locations. We extract modified locations directly from the goldpatch in the datasets,
...
present any process restrictions in our prompt, encouraging Repo Navigator to plan, call tools, and make decisions
...
The entry file of the code base is: [Relevant Path To Entry Point] [Entry Point]
