...
Locating the files and functions requiring modifi 

cation in large open-source software(OSS)repos 
...
mitigate these issues,post-training paradigms such as Super
...
date patches inside a dedicated Docker environment for
...
beyond any LLM ’s context window —localization drasti 
...
et al.,2025;He et al.,2025)typically rely on multiple **2.2.Software Engineering Agents**

tools,including S e a r chCl a s s,S e a r chMethods,and The introduction of SWE-bench(Jimenez et al.,2023;Yang

Get Impo rt s .Although effective to some extent,these
...
classes or inheritance,disappear after compilation,leav 
...
dencies,we focus on enhancing their ability to j ump across
...
we introduce a single,structurally grounded tool:j ump, grated commit history as agent memory.RepoLens(Wang
...
itory to enable repo-level understanding.These pipelines
...
complex tools while reasoning(Li et al.,2024;Huang et al., ORCALOCA(Yu et al 2025b)which enhances efficiency
...
Nevertheless,prior agents overlook the structural relations *Y* *∗* =*{*(*f**i**,g**i j*)*}*,where *g**i j* denotes a function or code
...
gle execution-logic-focused tool,reducing usage complexity. The objective is max*θ* E*τ ∼π**θ*[*R*(*τ*)] .
...
both RepoSearcher(Ma et al.,2025)and LocAgent(Chen RepoNavigator uses a *single-tool* design to avoid multi 
...
servations(code snippets or error messages),then continues

reasoning until termination.The loop is *reason → act →*

for repository-level issue localization.The method con 

sists of three components:(1)a unified tool to retrieve the

*observe*.

definition of any symbols in a given file,(2)a reasoning –

action agent loop that alternates between natural-language **3.3.Jump:Symbol Resolution**
...
jectories.Below we provide the formal problem setting and

mates Python ’s runtime name-binding semantics.Given a

the detailed method.

symbol occurrence *s* at source location *ℓ*,Pyright computes
...
position corresponding to a valid definition site of *s*.In function:

practice,we use fi l e path and symbol to resolve *ℓ*.If

we have multiple symbols with the same name exist in the

tsoaomlewchoidcehsanlilpopwest,fworeaacdcduirtaitoenraellsyolpuatirosen aonf*ℓ*i nde x to the
...
sis,function return types,and stub files( .pyi).Member
...
port dependency graph that statically emulates Python ’s

module loading semantics is built.Import statements intro  **4.1.Experimnent Setup**
...
filtering.Resolution may therefore traverse multiple mod  Qwen2.5 -7B -Instruct with RepoNavigator to sample each

ules before reaching a concrete definition.
...
free performance among same-size models;yellow background illustrates training-free RepoNavigator;blue background illustrates
...
same-size models;yellow background illustrates training-free RepoNavigator;blue background illustrates RepoNavigator trained with
...
cantly lower precision score than RepoNavigator,and result

in lower S -F1 and IoU.This indicates that RepoNavigator

behaves more conservatively and generates less wrong lo 

cSaOtiToAns.For 14Bllandi3i2B mf odels,RhepdoNTavhiigaitor alichievhes
...
less(Xia et al.,2024),Orcaloca(Yu et al.,2025b),and Compared with RepoSearcher,which is distilled from
...
GRPO,trained RepoNavigator outperforms it on all metri  performance for RepoNavigator,both before and after re 
...
probably due to the simplified tool we integrate to the agent in this context
...
influence of data leakage in SWE-bench Verified,we can localization front-end with other methods.Table.3 illus 
...
has the highest performance on issue resolution,while rein 

forcement learning improves its performance furthermore.
...
RFT-only and RFT+GRPO.Moreover,although RFT has ac  In this section,we analyze the logic behind RepoNaviga 
...
sembled functions is more effective than building multiple

task-specific tools.

sion contradicts with previous SWE agents trained with RL

(Ma et al.,2025),however,it aligns with the broader field of

reinforcement learning,where RFT and SFT(as a cold start) **5.1.Impact on the Action Space ofAgents**
...
ity of the *i*-th call be *p**i*.For a task that requires *k* sequential RepoNavigator and conduct RL training with only the out 
...
scope tools executed in sequence.
...
bol definitions,its access scope can be obtained by starting
...
cl aude -3 -7 -s onnet, February 2025.
...
Prasanna,V.,Cohan,A.,and Wang,X.LocAgent:Graph 

guided LLM agents for code localization.In Che,W.,

Nabende,J.,Shutova,E.,and Pilehvar,M.T.(eds.),*Pro *

*ceedings ofthe 63rdAnnual Meeting ofthe Association*
...
programming –the rise of code intelligence.*arXivpreprint*
...
*ofthe IEEE/CVF conference on computer vision andpat *
...
Liu,Z. Cosil:Software issue localization via llm 
...
Zamani,Hd.l,and Han,J.hSearich-r1:iThrainiinfg llms to Wang,X.,Li,B.,Song,Y.,Xu,F.F.,Tang,X.,Zhuge,
...
*on Learning Representations*,2025a. URL http s:
...
P.(ed.),*Proceedings ofthe 17th International Conference* Wang,Y.,Mao,W.,Wang,C.,Zhou,Z.,Zhou,Y.,Zhao,W.,
...
locate software issues.*arXivpreprint arXiv:2509.21427*,

2025b.

Li,Y.,Wen,H.,Wang,W.,Li,X.,Yuan,Y.,Liu,G.,Liu,

J.,Xu,W.,Wang,X.,Sun,Y.,et al.Personal llm agents:Xi C S D Y D S d Zh L A l D
...
C Deng C Zhang C Ruan C et al Deepseek-v3 Yan,Y.,Wang,S.,Huo,J.,Yu,P.S.,Hu,X.,and Wen,Q.
...
framework with agent team optimization.*arXiv preprint* Yang,A.,Li,A.,Yang,B.,Zhang,B.,Hui,B.,Zheng,B.,
...
http s://p retty -radi o -b7 5.not i on .s it e/

D e epSWE -T rai ni ng -a -Ful ly -Ope n -s ou r ced -St atsey-stoemf -stghenee-rAalrizte-tCoovdisiunagl -soAftgweanrte -dbomy -aiSncs?al*a*i*r*n*X*g*iv*-RL -2 2 2 8 1 9 0 2 c1 4 6 8 1 93 aabbe 9 a8 c5 9bbe3 3,
...
llm reinforcement learning system at scale.*arXivpreprint* **Agentless** Agentless(Xia et al.,2024)is a workflow for
...
//a rxiv .o rg/ab s/2 5 0 2.0 0 3 5 0.
...
J.,Wang,C.,Fan,T.,Du,Z.,et al.Vapo:Efficient and **LocAgent** LocAgent(Chen et al.,2025)is almost a fully 
...
nated into the context at the beginning of the searching
...
level localization,which aligns with CoSIL.RepoSearcher

introduced the first training framework *ToolTrain* for lo 
...
to zero.We use the function-level localization result of **C.Threats to Validity**
...
**Implementation** When the response exceeds the maxi  into consideration.However,using golden patches is ac 
...
mum tool-calling times is not achieved and the final answer
...
thalitddi atafleakage iatithe preh-trdainNing phhrasle threatensl thde **D.Case Study**

va ty o post-tra n ng met o s. evert e ess,we exc u e

this concern by results in Tabel.2.The SWE-bench Pro In this section,we present the full trajectory of RepoNavi 
...
in the training dataset if the repository also appears in SWE  (Shen 2024)and present an example Noted we do not
...
ing RepoNavigator to plan,call tools,and make decisions

**Language Server** In practice,we apply a Python lan  full-automatically.This is distinct with Agentless(which

guage server to extract the definition code corresponding has a fixed workflow),LocAgent(which predefines a spe 
...
You a re Qwe n, c re at ed by Alibaba Cl oud . You a re a helpful a s s i st ant .

#T o ol s

You may c al l one o r mo re fun ct i on s t o a s s i st with the u s e r que ry .

You a re p rovided with fun ct i on s ignat u re s withi n <t o ol s></t o ol s>XML t ag s:

<t o ol s>

{"type": "fun ct i on", "fun ct i on":{"name": "che ck", "de s c ript i on": "I n the

spe ci fi c fi l e path, a symbol i s re fe r red and thi s t o ol c an fi nd

whe re the t o ol i s de fi ned .

F o r i n st an ce, i n the fi r st t u rn, fi l e path i s the e nt ry poi nt of .",

_

"pa ramet e r s":{"p rope rt i e s":{"symbol":{"de s c ript i on": "The symbol who s e

de fi nit i on c ode wi l l be give n t o the age nt .", "type": "st ri ng"}, "fi l e path":

_

{"de s c ript i on": "The rel evant path t o the fi l e whe re the symbol i s re fe r red .",
...
{"name": <fun ct i on -name>, "a rgume nt s": <a rg s -j s on -obj e ct>}
...
The ’ fi l e path ’ i s the rel evant path of whe re the symbol i s c al l ed,

_

NOT whe re it i s de fi ned!
