...
proposed. To achieve these objectives, some challenges need to be overcome.
...
struggle to accurately map screenshots to precise coordinates for their actions and lack deep understanding of basic graphical user interface (GUI) interactions and application-specific features.
...
available to LLM agents can be broadly categorized based on the type of infor- mation they help perceive:
...
directly affects the reasoning and planning modules. Therefore, continuous advancements in perception technologies are not merely improvements to one component, but fundamental enablers for building more intelligent, reliable, and capable LLM agents.
...
correcting its reasoning process, learning better ways to use tools, updating its "memory" or state [49], or generating a revised plan or a new set of actions [6,38].
...
**-Memory Management Expert:** Responsible for handling the agent's memory. This expert ensures that relevant information is retrieved efficiently and that the agent's context is maintained effectively, which is a critical challenge in LLM-based multi-agent systems [23,33].
...
(AWM) is a method that induces commonly reused routines (workflows) from training examples and then selectively provides these workflows to the agent to guide subsequent generations [59].
...
Gidey, H.K., Marmsoler, D.: FACTum (2018) Gidey, H.K., Marmsoler, D., Ascher, D.: Modeling adaptive self-healing systems. CoRR abs/2304.12773 (Apr 2023). https://doi.org/10.48550/arXiv.2304.12773,
