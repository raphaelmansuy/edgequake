...
**55555555555555555555555555555555 55555555555555555555555555555555 55555 6**

 **55555555555555555555555555555555 55555555555555555555555555555555 55555555555555 6**

### 55555555555555555555555555555555 55555555555555555555555555555555 A

**55555555555555555555555555555555 55555555555555555555555555555555 5555555555555 E**
...
**55555555555555555555555555555555 55555555555555555555555555555555 55555555555555555 6I**

 **55555555555555555555555555555555 55555555555555555555555555555555 5555555555555555 6I**

### 55555555555555555555555555555555 55555555555555555555555555555555 55555 A6

### 55555555555555555555555555555555 55555555555555555555555555555555 5555555555 A6

**55555555555555555555555555555555 55555555555555555555555555555555 55555555555 AA**
...
of this feature. T he other available public 
...
This document tries to close this gap by 

this technology. It is a work in prog ress
...
applications that process potentially 

## 1Profiles

https://media.blackhat.com/bh -dc -11/Blazakis/Black Hat_DC_2011_Blazakis_Apple_Sandbox

## 2Profiles

Leopard version of Mac OS X. It was called Seatbelt 
...
references are Apple's own sandbox profiles (located at 
...
trying to document the operations and options available in 

and based on Snow Leopard v10.6.8. Apple still considers 

.

ll contributions and fixes 

reverser@put.as or tweet @osxreverser.
...
untrusted input such as pd f, word/excel/powerpoint 
...
There are two alternatives to use this feature
...
- kSBXProfile NoInternet : TCP/IP netw
- kSBXProfile NoNetwork : All sockets
- kSBXProfile NoWrite : File system writes are prohibited.
- kSBXProfile NoWrite Except Temporary : File system writes are restricted to the temporary folder

/var/tmp and the folder specified by theconfstr(3) configuration variable 

_CS_DARWIN_USER_TEMP_DIR.

- kSBXProfile Pure Computation : All operating system services are prohibited.
...
http://developer.apple.com/library/mac/#documentation/Security/Conceptual/Code Signing Guide/Introduction/Introduction.html#//a
...
tom profiles. Either you can use the absolute path to 

, as long it is located at one of the these folders:
...
kSBXProfile NoInternet

kSBXProfile NoNetwork

kSBXProfile NoWrite Except Temporary

kSBXProfile NoWrite

kSBXProfile Pure Computation
...
while others are binary only (allow or deny).
...
" import " command, for example a profile with common 
...
file write xattr is implemented in a few kernel functions). This is described in the ite m Applies to,
...
getattrlist_internal, link, namei, readlink, Check Access, vn_stat 
...
Controls access to revoke (v oid all references to file by ripping underlying filesystem away from 
...
g etvolattrlist, access1, link, __mac_mount,vn_open_auth, union_dir check, fcntl_nocancel, 
...
--- S ------ 1 root staff 9 Sep 12 22:46 /Users/ reverser/test suid 
...
Sep 2 20:21:19 macbox sandboxd[43908]: umount(43911) deny file write unmoun t
...
Sep 2 00:38:13 macbox sandboxd[25217]: Python(25216) deny file
...
sem_open, sem_post, sem_unlink

functions (create, open, post, unlink, wait).

, sem_wait_nocancel, sem_trywait
...
(global name regex #"^com.apple.Device Link.Apple Mobile Backup*")
...
soo_read, soreceive, recvit, listen, b in d, unp_bind, connect_nocancel, sendit, soo_write, 
...
Sep 2 22:29:03 macbox sandboxd[47760]: nc(47758) deny ne twork outbound
...
second level name bpf_bufsize in debug.bpf_bufsize i s invalid 
...
Sep 2 22:49:30 macbox sandboxd[48428]: d

sysctlbyname and sysctlnametomib.

, where a (deny 

            - read), even if we have a (allow default).

fsctl, setlcid, nfssvc, reboot, s ettimeofday, adjtime, 

2200

ate(48435) deny system set time
...
from the standpoint of the MAC framework and since these operations can affect ma ny aspects of 
...
control check is called when the mac_set_lctx/lcid syste m call is invoked. A user space 
...
This will deny all file read access to /dev only, but everything else inside /dev is n't protected by 
...
§ (allow network* (remote unix socket (path
...
ca n be used instead of "local", "remote", and "unix".
...
arch: /Applications/i Tunes.app/Contents/MacOS/i Tunes.orig isn't executable
...
denied.
                ...
                - simplify to create a simplified sandbox profile based on
...
You will need to replace the string %username% with yours. A global search a nd replace does the 
...
(allow file read*
 ...
 #"^/Library/Preference Panes/Growl.pref Pane/*" 
...
(allow file read*
 ...
 #"^/Users/%username%/Library/Input Managers/*" 
 ...
 (literal "/private/var/db/mds/messages/se_Security Messages") 
 ...
 (literal "/Users/%username%/Library/Preferences/com.apple.Launch Services.plist") 
...
## Page 40
 ...
 (literal "/Library/Preference Panes") 
...
; allow read to preferences files 
 ...
 (regex #"^/Users/%username%/Library/Preferences/By Host/.Global Preferences.*") 

 (literal "/Users/%username%/Library/Preferences/.Global Preferences.plist") 

 (literal "/Users/%username%/Library/Preferences/uk.co.opencommunity.vienna2.plist") 

 (literal "/Library/Preferences/.Global Preferences.plist") 
...
(allow file read*
 ...
 (literal "/Users/%username%/Library/Preferences/com.apple.java.Java Preferences.plist") 
...
(allow file write data
 ...
 (literal "/Users/%username%/Library/Icons/Webpage Icons.db") 

(allow file write*

(literal "/Users/%username%/Library/Icons/Webpage Icons.db journal")
...
(global name "com.apple.Core Services.coreservicesd")

(global name "com.apple.Security Server")
...
(global name "com.apple.System Configuration.PPPController")

(global name "en (Apple)_Open Step")

(global name "com.apple.system.Directory Service.libinfo_v1")
...
(global name "com.apple.Font Server")

(global name "com.apple.pasteboard.1")

(global name "com.apple.tsm.uiserver")

(global name "com.apple.System Configuration.configd")

(global name "com.apple.Voice Over.running")

(global name "com.apple.Font Objects Server")

(global name "com.apple.FSEvents")

(global name "com.apple.cvms Serv")

(global name "Growl Application Bridge Pathwa y")
...
These are some links and books where you can fi nd further information about this technology. The 
...
dumped to IDA console (you can mo dify to write to a file, if you wish). 
...
file = Ask File(0, " ", "Load the file wi th all operations names:"); 

// load the array with the operations names 

myarray = Create Array("operationsnames"); 

// if it doesn't exist, then load information 

 Message("Array doesn't exist; loading data... \ n"); 

 myarrayid = Get Array Id("operationsnames"); 
...
// Message("Operation %s", operation); 
 ...
 kr = Set Array St ring(myarrayid, myindex, operation); 
...
while (ea != BADADDR) 
 ...
 Message("%s;%s", Get Function Name(ea), Get Array Element(AR_STR, myarrayid, 

xtol(register2))); 

 // iterate to the next call... 

 crap = Find Code(ea, SEARCH_ UP | SEARCH_CASE | SEARCH_NEXT); 

// CRED_CHECK_SOCKET 

//first = 0x2596; 

first = Loc ByName("_cred_check_socket"); 
...
first = Loc ByName("_cred_check_vnode"); 
...
while (ea != BADADDR) 
 ...
 Message("%s;%s", Get Function Name(ea), Get Array Elemen t(AR_STR, myarrayid, 

// sb_evaluate 

//first = 0x34DC; 

first = Loc ByName("_sb_evaluate"); 
