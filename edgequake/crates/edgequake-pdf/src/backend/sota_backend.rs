...
impl SotaBackend {
    ...
    /// Extract text from content stream using font encodings
    ...
    fn group_two_column_layout(
        ...
        let affiliation_threshold = 140.0; // Elements at Y < 140 but > footer are affiliations
    ...
    fn lines_to_blocks(
        ...
        let section_pattern = regex::Regex::new(r"^(\d+\.)+\s+[A-Z]").ok();
    ...
    fn extract_page(
        ...
        let elements = self.extract_text_elements(&content_bytes, &fonts)?;
        debug!("Page {} has {} text elements", page_num, elements.len());

        // Group into lines (handles two-column layouts) and get column bounding boxes
        let (lines, columns) = self.group_into_lines(elements, width, height);
        debug!(
        ...
        // Convert to blocks
        let blocks = self.lines_to_blocks(lines, width, height);
