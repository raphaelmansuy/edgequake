//! Diagnostic tool to trace raw text elements before grouping
//!
//! Usage: cargo run --bin trace_elements -- <pdf_path>
                        ...
                        if !op.operands.is_empty() {
                            if let Object::String(bytes, _) = &op.operands[0] {
                                ...
                                if element_count <= 100 {
                                    println!(
                                        "Elem {:3}: X={:7.2} Y={:7.2} text='{}' matrix=[{:.1},{:.1},{:.1},{:.1}]",
                                        element_count,
                                        text_matrix[4],
                                        text_matrix[5],
                                        text,
                                        text_matrix[0], text_matrix[1], text_matrix[2], text_matrix[3]
                                    );
                                }
                            }
                        }
