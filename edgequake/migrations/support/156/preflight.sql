-- M156 preflight: count rows that need source_ids backfill.
DO $$
DECLARE
  v_graph text;
  v_nodes bigint;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'age') THEN
    RAISE NOTICE 'M156 preflight: AGE not installed';
    RETURN;
  END IF;
  FOR v_graph IN SELECT name FROM ag_catalog.ag_graph ORDER BY name LOOP
    ...
    RAISE NOTICE 'M156 preflight: % needs nodes=% edges=%', v_graph, v_nodes, v_edges;
  END LOOP;
END
$$;
