    ...
    group.bench_function("single_vector", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let data = vec![
                    ("vec-1".to_string(), embedding, json!({"content": "test"})),
                ];
                    ...
                    let data: Vec<_> = (0..*batch_size)
                        .map(|i| {
                            let embedding: Vec<f32> = (0..384)
                                ...
                                .collect();
                            (format!("vec-{}", i), embedding, json!({"content": format!("test {}", i)}))
                        })
                        .collect();
        ...
        let data: Vec<_> = (0..500)
            .map(|i| {
                let embedding: Vec<f32> = (0..384)
                    .map(|j| ((i + j) as f32 / 1000.0).sin())
                    .collect();
                (format!("vec-{}", i), embedding, json!({"content": format!("document {}", i)}))
            })
            .collect();
    ...
    group.bench_function("upsert_node", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                storage.upsert_node(black_box("entity-1"), props).await.unwrap();
            })
        })
    });
    ...
    group.bench_function("upsert_edge", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                storage.upsert_node("entity-1", node_props.clone()).await.unwrap();
...
fn bench_graph_traversal(c: &mut Criterion) {
    ...
    let storage = rt.block_on(async {
        ...
        for i in 0..100 {
            let mut props: HashMap<String, serde_json::Value> = HashMap::new();
            props.insert("type".to_string(), json!("ENTITY"));
            storage.upsert_node(&format!("entity-{}", i), props).await.unwrap();
        }
    ...
    group.bench_function("get_node", |b| {
        b.iter(|| {
            rt.block_on(async {
                storage.get_node(black_box("entity-50")).await.unwrap()
            })
        })
    });
    ...
    group.bench_function("get_neighbors", |b| {
        b.iter(|| {
            rt.block_on(async {
                storage.get_neighbors(black_box("entity-50"), 1).await.unwrap()
            })
        })
    });
...
fn bench_kv_operations(c: &mut Criterion) {
    ...
    group.bench_function("upsert_single", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let data = vec![
                    ("key-1".to_string(), json!({"content": "test value"})),
                ];
                storage.upsert(black_box(&data)).await.unwrap();
            })
        })
    ...
    group.bench_function("get_single", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                storage.get_by_id(black_box("key-1")).await.unwrap()
            })
        })
    });
