...
describe('WebSocket Client', () => {
  ...
  describe('isValidWebSocketUrl', () => {
    it('accepts ws:// URLs', () => {
      expect(isValidWebSocketUrl('ws://localhost:3000/ws/progress/track-1')).toBe(true);
    });

    it('accepts wss:// URLs', () => {
      expect(isValidWebSocketUrl('wss://api.example.com/ws/progress/track-1')).toBe(true);
    });
...
describe('Reconnection Scenarios', () => {
  ...
  describe('reconnect decision matrix', () => {
    ...
    it('reconnects on transient error', () => {
      const shouldRetry = shouldReconnect(0, config, false) && isTransientError(1006);
