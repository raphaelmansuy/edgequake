...
describe('classifyError', () => {
  describe('timeout errors', () => {
    it('classifies timeout_error as warning', () => {
      const result = classifyError({ code: 'timeout_error', message: 'Request timed out' });
      expect(result.severity).toBe('warning');
      expect(result.suggestions).toContain('Check your internet connection');
    });

    it('classifies network_timeout as warning', () => {
      const result = classifyError({ code: 'network_timeout', message: 'Network unavailable' });
      expect(result.severity).toBe('warning');
    });
  });
  ...
  describe('rate limit errors', () => {
    it('classifies rate_limit as warning', () => {
      const result = classifyError({ code: 'rate_limit', message: 'Too many requests' });
      expect(result.severity).toBe('warning');
      expect(result.suggestions).toContain('Too many requests - please wait 30 seconds');
    });

    it('classifies 429 errors as warning', () => {
      const result = classifyError({ code: 'http_429', message: 'Rate limit exceeded' });
      expect(result.severity).toBe('warning');
    });
  });
  ...
  describe('parse errors', () => {
    it('classifies parse_error as error', () => {
      const result = classifyError({ code: 'parse_error', message: 'Failed to parse PDF' });
      expect(result.severity).toBe('error');
      expect(result.suggestions).toContain('The PDF might be corrupted or password-protected');
    });

    it('classifies corrupt_file as error', () => {
      const result = classifyError({ code: 'corrupt_file', message: 'File is corrupted' });
      expect(result.severity).toBe('error');
    });
  });
  ...
  describe('LLM errors', () => {
    it('classifies llm_error as warning', () => {
      const result = classifyError({ code: 'llm_error', message: 'LLM API failed' });
      expect(result.severity).toBe('warning');
      expect(result.suggestions).toContain('The AI model encountered an issue processing this content');
    });

    it('classifies extraction_failed as warning', () => {
      const result = classifyError({ code: 'extraction_failed', message: 'Entity extraction failed' });
      expect(result.severity).toBe('warning');
    });
  });
  ...
  describe('storage errors', () => {
    it('classifies storage_error as critical', () => {
      const result = classifyError({ code: 'storage_error', message: 'Database write failed' });
      expect(result.severity).toBe('critical');
      expect(result.suggestions).toContain('There was an issue saving to the database');
    });

    it('classifies database_error as critical', () => {
      const result = classifyError({ code: 'database_connection_lost', message: 'DB connection lost' });
      expect(result.severity).toBe('critical');
    });
  });
  ...
  describe('unknown errors', () => {
    it('classifies unknown errors as error', () => {
      const result = classifyError({ code: 'unknown', message: 'Something went wrong' });
      expect(result.severity).toBe('error');
    });
    ...
    it('provides default suggestions when none provided', () => {
      const result = classifyError({ code: 'mystery', message: 'Mystery error' });
  ...
  describe('case insensitivity', () => {
    it('handles uppercase error codes', () => {
      const result = classifyError({ code: 'TIMEOUT_ERROR', message: 'Timeout' });
      expect(result.severity).toBe('warning');
    });

    it('handles mixed case error codes', () => {
      const result = classifyError({ code: 'Rate_Limit_Exceeded', message: 'Rate limited' });
      expect(result.severity).toBe('warning');
    });
  });
  ...
  describe('empty/null handling', () => {
    ...
    it('handles undefined suggestions', () => {
      const result = classifyError({ code: 'unknown', message: 'No suggestions' });
