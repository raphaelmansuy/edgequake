...
import type { IngestionStage, StageProgress, IngestionProgress } from '@/types/ingestion';
...
function createDefaultStages(currentStage?: IngestionStage) {
  ...
  return stageOrder.map(stageId => ({
    id: stageId,
    label: stageId,
    status: stageId === currentStage ? 'running' as const : 'pending' as const,
  }));
}
 ...
 */
function buildStagesFromProgress(progress: Partial<IngestionProgress> | null): Array<{
  id: IngestionStage;
  label: string;
  ...
  return stageOrder.map(stageId => {
    const stageData = progress.progress!.stages.find((s: StageProgress) => s.stage === stageId);
 ...
 */
function getCurrentMessage(progress: Partial<IngestionProgress> | null): string {
  ...
  const stageData = progress.progress.stages.find((s: StageProgress) => s.stage === currentStage);
  ...
  describe('without progress data', () => {
    ...
    it('should return default stages when stages array is missing', () => {
      const stages = buildStagesFromProgress({
        progress: { current_stage: 'chunking' } as unknown as IngestionProgress['progress'],
      });
    ...
    it('should mark completed stages correctly', () => {
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          stages: [
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1, duration_ms: 1000 },
            { stage: 'chunking', status: 'completed', progress: 100, completed_items: 10, total_items: 10, duration_ms: 2000 },
            { stage: 'extracting', status: 'running', progress: 30, completed_items: 3, total_items: 10 },
            { stage: 'merging', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'embedding', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'indexing', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          ],
        },
      };
    ...
    it('should include progress percentage for current stage only', () => {
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          stages: [
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
            { stage: 'chunking', status: 'completed', progress: 100, completed_items: 10, total_items: 10 },
            { stage: 'extracting', status: 'completed', progress: 100, completed_items: 50, total_items: 50 },
            { stage: 'merging', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
            { stage: 'embedding', status: 'running', progress: 60, completed_items: 30, total_items: 50 },
            { stage: 'indexing', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          ],
        },
      };
    ...
    it('should include duration for completed stages', () => {
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          latest_message: 'Extracting...',
          stages: [
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1, duration_ms: 1500 },
            { stage: 'chunking', status: 'completed', progress: 100, completed_items: 10, total_items: 10, duration_ms: 3200 },
            { stage: 'extracting', status: 'running', progress: 25, completed_items: 5, total_items: 20 },
            { stage: 'merging', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'embedding', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'indexing', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          ],
      ...
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          stages: [
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
            { stage: 'chunking', status: 'completed', progress: 100, completed_items: 10, total_items: 10 },
            { stage: 'extracting', status: 'failed', progress: 25, completed_items: 5, total_items: 20 },
            { stage: 'merging', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'embedding', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'indexing', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          ],
    ...
    it('should include stage message', () => {
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          stages: [
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1, message: 'Validated PDF' },
            { stage: 'chunking', status: 'running', progress: 50, completed_items: 5, total_items: 10, message: 'Page 5 of 10' },
            { stage: 'extracting', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'merging', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'embedding', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'indexing', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          ],
      ...
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          stages: [
            ...
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
            { stage: 'chunking', status: 'running', progress: 40, completed_items: 4, total_items: 10 },
          ],
        },
      };
  ...
  it('should return stage message when available', () => {
    expect(getCurrentMessage({
      progress: {
        ...
        stages: [
          { stage: 'extracting', status: 'running', progress: 50, completed_items: 5, total_items: 10, message: 'Extracting entities from chunk 5/10' },
        ],
      },
...
describe('Progress Completion Detection', () => {
  ...
  it('should detect processing status', () => {
    ...
    expect(progress.status !== 'completed' && progress.status !== 'failed').toBe(true);
  });
});
    ...
    const progress: Partial<IngestionProgress> = {
      progress: {
        ...
        stages: [
          ...
          { stage: 'embedding', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
          { stage: 'chunking', status: 'running', progress: 30, completed_items: 3, total_items: 10 },
        ],
      },
    };
  ...
  it('should handle 0% progress', () => {
    const stages = buildStagesFromProgress({
      progress: {
        ...
        stages: [
          { stage: 'preprocessing', status: 'running', progress: 0, completed_items: 0, total_items: 1 },
        ],
      },
    });
    ...
    const progress: Partial<IngestionProgress> = {
      progress: {
        ...
        stages: [
          ...
          { stage: 'extracting', status: 'completed', progress: 100, completed_items: 50, total_items: 50 },
          { stage: 'merging', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
          { stage: 'embedding', status: 'completed', progress: 100, completed_items: 50, total_items: 50 },
          { stage: 'indexing', status: 'completed', progress: 100, completed_items: 50, total_items: 50 },
        ],
      },
    };
  ...
  it('should handle unknown stage in progress data', () => {
    const progress: Partial<IngestionProgress> = {
      progress: {
        ...
        stages: [
          { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
          { stage: 'unknown_stage' as IngestionStage, status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
          { stage: 'chunking', status: 'running', progress: 50, completed_items: 5, total_items: 10 },
        ],
      },
    };
  ...
  it('should handle negative progress values', () => {
    const stages = buildStagesFromProgress({
      progress: {
        ...
        stages: [
          { stage: 'extracting', status: 'running', progress: -5, completed_items: 0, total_items: 10 },
        ],
      },
    });
  ...
  it('should handle progress values over 100', () => {
    const stages = buildStagesFromProgress({
      progress: {
        ...
        stages: [
          { stage: 'embedding', status: 'running', progress: 120, completed_items: 60, total_items: 50 },
        ],
      },
    });
      ...
      const progress: Partial<IngestionProgress> = {
        progress: {
          ...
          stages: [
            { stage: 'preprocessing', status: 'completed', progress: 100, completed_items: 1, total_items: 1 },
            { stage: 'chunking', status: 'completed', progress: 100, completed_items: 10, total_items: 10 },
            { stage: 'extracting', status: 'running', progress: i / 10, completed_items: i, total_items: 1000 },
            { stage: 'merging', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'embedding', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
            { stage: 'indexing', status: 'pending', progress: 0, completed_items: 0, total_items: 0 },
          ],
