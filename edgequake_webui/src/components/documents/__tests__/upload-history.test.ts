 ...
 */
function filterByStatus(items: HistoryItem[], filter: HistoryFilter): HistoryItem[] {
 ...
 */
function countByType(items: HistoryItem[]): { success: number; failed: number } {
  return {
...
describe('searchItems', () => {
  const items: HistoryItem[] = [
    { type: 'success', trackId: 'abc-123', documentId: 'doc-xyz', documentName: 'Report.pdf', timestamp: new Date() },
    { type: 'success', trackId: 'def-456', documentId: 'doc-abc', documentName: 'Invoice.pdf', timestamp: new Date() },
...
describe('integration scenarios', () => {
  it('filters and searches combined', () => {
    const items: HistoryItem[] = [
      { type: 'success', trackId: 'report-abc', documentName: 'Report.pdf', timestamp: new Date() },
      { type: 'failed', trackId: 'report-def', documentName: 'Report2.pdf', timestamp: new Date() },
      { type: 'success', trackId: 'invoice-123', documentName: 'Invoice.pdf', timestamp: new Date() },
    ];
  ...
  it('full pipeline: filter -> search -> sort -> limit', () => {
    const now = new Date();
    const items: HistoryItem[] = [
      { type: 'success', trackId: 'a-report', timestamp: new Date(now.getTime() - 1000) },
      { type: 'success', trackId: 'b-report', timestamp: now },
      { type: 'success', trackId: 'c-report', timestamp: new Date(now.getTime() - 2000) },
