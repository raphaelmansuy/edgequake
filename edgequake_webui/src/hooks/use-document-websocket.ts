...
const PROCESSING_STATUSES = ['processing', 'chunking', 'extracting', 'embedding', 'indexing'] as const;
  ...
  useEffect(() => {
    ...
    const processingDocs = documents.filter(
      (doc: Document) => 
        doc.track_id && 
        doc.status && 
        PROCESSING_STATUSES.includes(doc.status as typeof PROCESSING_STATUSES[number])
    );
    ...
    console.log('[useDocumentWebSocket] Subscribed to', trackIds.length, 'processing documents');

    // Unsubscribe when hook dependencies change
    return () => {
      unsubscribe(trackIds);
      console.log('[useDocumentWebSocket] Unsubscribed from', trackIds.length, 'documents');
    };
