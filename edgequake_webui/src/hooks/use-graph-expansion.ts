...
export function useGraphExpansion() {
  ...
  const handleNodeExpand = useCallback(async (nodeId: string) => {
    ...
    try {
      ...
      if (expandedNodes.has(nodeId)) {
        toast.info(t('graph.expand.alreadyExpanded', 'Node already expanded'));
        return;
      }
      ...
      if (newNodes.length === 0) {
        toast.info(t('graph.expand.noNewNodes', 'All neighbors already in graph'));
      ...
      animateNodes(sigmaGraph, newPositions, { duration: 300, easing: 'quadraticInOut' });
      ...
      toast.success(
        t('graph.expand.success', 'Added {{count}} new nodes', { count: newNodes.length })
      );
    } catch (error) {
   ...
   */
  const handleNodePrune = useCallback((nodeId: string) => {
    if (isOperatingRef.current) return;
    isOperatingRef.current = true;
    setIsPruning(true);
    ...
    try {
      ...
      if (sigmaGraph.order === 1) {
        toast.error(t('graph.prune.cannotDeleteLast', 'Cannot delete the last node'));
        return;
      }
      ...
      toast.success(
        t('graph.prune.success', 'Removed {{count}} nodes', { count: nodesToDelete.size })
      );
    } catch (error) {
