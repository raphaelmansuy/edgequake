...
const PHASE_LABELS = {
  ...
  pdf_conversion: { label: "PDF → Markdown", description: "Converting PDF pages to text" },
  chunking: { label: "Chunking", description: "Splitting text into chunks" },
  embedding: { label: "Embedding", description: "Generating vector embeddings" },
  extraction: { label: "Extraction", description: "Extracting entities and relationships" },
  graph_storage: { label: "Storage", description: "Storing in knowledge graph" },
};
...
describe('extractErrorCode', () => {
  it('extracts timeout_error', () => {
    expect(extractErrorCode("Connection timeout after 30s")).toBe("timeout_error");
    expect(extractErrorCode("Request TIMEOUT")).toBe("timeout_error");
  });

  it('extracts network_error', () => {
    expect(extractErrorCode("Network error: connection refused")).toBe("network_error");
...
describe('getFailedPhaseName', () => {
  it('returns undefined when no phase failed', () => {
    const phases: PhaseInfo[] = [
      { phase: "upload", label: "Upload", description: "", status: { type: "completed" }, index: 0 },
      { phase: "pdf_conversion", label: "PDF → Markdown", description: "", status: { type: "active", current: 1, total: 5, percent: 20 }, index: 1 },
    ];
  ...
  it('returns label of failed phase', () => {
    const phases: PhaseInfo[] = [
      { phase: "upload", label: "Upload", description: "", status: { type: "completed" }, index: 0 },
      { phase: "pdf_conversion", label: "PDF → Markdown", description: "", status: { type: "failed", error: "Parse error" }, index: 1 },
    ];
