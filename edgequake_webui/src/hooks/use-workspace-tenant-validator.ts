...
export function useWorkspaceTenantValidator(options?: {
  ...
  const { selectedTenantId, selectedWorkspaceId, workspaces, selectWorkspace, reset } = useTenantStore();
  ...
  useEffect(() => {
    ...
    const validateAndCorrect = async () => {
      ...
      try {
        ...
        const workspaceInList = workspaces.find(w => w.id === selectedWorkspaceId);
        ...
        if (workspaceInList) {
          ...
          if (workspaceInList.tenant_id !== selectedTenantId) {
            ...
            console.error('[WorkspaceTenantValidator] Mismatch detected:', result.reason);
            ...
            if (autoCorrect) {
              ...
              if (firstWorkspace) {
                ...
                console.warn('[WorkspaceTenantValidator] No valid workspace found for tenant, resetting');
                reset();
                return;
              }
            }
        ...
        if (workspace.tenant_id !== selectedTenantId) {
          ...
          console.error('[WorkspaceTenantValidator] API verification failed:', result.reason);
          options?.onValidationFailed?.(result);
