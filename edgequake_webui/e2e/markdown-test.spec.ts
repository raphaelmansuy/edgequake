...
test.describe('Markdown Rendering Test', () => {
  test('should properly render markdown formatting in responses', async ({ page }) => {
    ...
    const hasFormattingResponse = pageText?.toLowerCase().includes('bold') ||
    ...
    const hasFormattingElements = strongCount > 0 || emCount > 0 || bCount > 0 || iCount > 0;
    ...
    const hasRawMarkdown = pageText?.includes('**bold**') || pageText?.includes('*italic*');
