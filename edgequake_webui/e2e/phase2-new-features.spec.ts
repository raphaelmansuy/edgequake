...
test.describe("Phase 2 UX Improvements - Documents Page", () => {
  ...
  test.describe("Bulk Selection", () => {
    test("select all checkbox should be visible in table header", async ({
      page,
    }) => {
    ...
    test("bulk actions bar appears when documents selected", async ({
      page,
    }) => {
      const rowCheckboxes = page
        .locator("tbody")
    ...
    test("clicking document row shows preview panel", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const rightPanel = page.locator(
          '[data-preview-panel], [role="complementary"]'
        );
        const previewContent = page.locator(
          "text=/Metadata|Content|Properties/i"
        );

        // Either right panel or preview content should be visible
        const isVisible =
          (await rightPanel.isVisible()) || (await previewContent.isVisible());
...
test.describe("Phase 2 UX Improvements - Query Page", () => {
  ...
  test.describe("Conversation History Panel", () => {
    ...
    test("search conversations input should be visible in history panel", async ({
      page,
    }) => {
      const searchInput = page.locator(
        'input[placeholder*="Search conversations" i]'
      );
      await expect(searchInput).toBeVisible({ timeout: 10000 });
    });
  ...
  test.describe("Query Empty State", () => {
    test("empty state shows welcome message when no messages", async ({
      page,
    }) => {
      // Look for empty state content
      const emptyState = page.locator(
        "text=/Start a conversation|Ask Anything/i"
      );
    ...
    test("example queries/suggestions should be visible", async ({ page }) => {
      ...
      const suggestions = page
        .locator("button")
        .filter({ hasText: /What are|How do|Show me/i });
...
test.describe("Phase 2 UX Improvements - Graph Page", () => {
  ...
  test.describe("Entity Browser Panel", () => {
    test("entity browser panel should be visible on left side", async ({
      page,
    }) => {
      // Look for entity browser panel
      const entityBrowser = page.locator(
        'aside, [aria-label*="Entity" i], [aria-label*="browser" i]'
      );
      const browserVisible = await entityBrowser
        .first()
        .isVisible()
        .catch(() => false);
      ...
      const collapsedVisible = await collapsedState
        .isVisible()
        .catch(() => false);
    ...
    test("entity browser has search input", async ({ page }) => {
      ...
      const searchInput = page.locator(
        'input[placeholder*="Search entities" i]'
      );
      await expect(searchInput).toBeVisible({ timeout: 10000 });
    });
  ...
  test.describe("Graph Controls", () => {
    ...
    test("reset view button should be visible", async ({ page }) => {
      const resetButton = page.locator(
        'button[title*="Reset" i], button[title*="Fit" i]'
      );
    ...
    test("layout control should be visible", async ({ page }) => {
      ...
      const layoutControl = page
        .locator("button")
        .filter({ hasText: /Layout|Force|Circular/i });
      const isVisible = await layoutControl
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test.describe("Graph Empty State", () => {
    test("shows empty state message when no graph data", async ({ page }) => {
      ...
      const emptyMessage = page.locator(
        "text=/No knowledge graph yet|No entities/i"
      );
...
test.describe("Phase 2 UX Improvements - Cross-Feature Integration", () => {
  ...
  test("collapsible panels work consistently across pages", async ({
    page,
  }) => {
    ...
    const queryCollapseButton = page
      .locator('button[aria-label*="Collapse" i]')
      .first();
    ...
    const graphCollapseButton = page
      .locator('button[aria-label*="Collapse" i]')
      .first();
