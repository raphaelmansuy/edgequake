...
test.describe('Phase 2 UX Improvements - Graph & Query', () => {
  ...
  test.describe('Graph Export', () => {
    test('export button should be visible in graph toolbar', async ({ page }) => {
      await page.goto('/graph');
      await page.waitForLoadState('networkidle');
      
      // Look for the export button (download icon)
      const exportButton = page.locator('button[title*="Export" i], button:has([aria-label*="Export" i])').first();
  ...
  test.describe('Graph Search', () => {
    test('search button with keyboard shortcut hint should be visible', async ({ page }) => {
      await page.goto('/graph');
      await page.waitForLoadState('networkidle');
      
      // Look for search button with Cmd+K shortcut hint
      const searchButton = page.locator('button').filter({ hasText: /K/i }).first();
  ...
  test.describe('Legend Type Filter', () => {
    test('graph controls should be visible in graph view', async ({ page }) => {
      ...
      const graphControls = page.locator('[aria-label*="Graph controls" i], [role="toolbar"]').first();
    ...
    test('graph page should load successfully', async ({ page }) => {
      ...
      const graphTitle = page.locator('h1, h2').filter({ hasText: /Knowledge Graph|Graph/i }).first();
      const graphContent = page.locator('[data-graph-container], .sigma-container, canvas').first();
  ...
  test.describe('Query Interface', () => {
    test('query textarea should be present', async ({ page }) => {
      ...
      const textarea = page.locator('textarea[placeholder*="Ask" i], textarea[placeholder*="question" i]');
    ...
    test('query history sidebar should show favorites and recent sections', async ({ page }) => {
      await page.goto('/query');
      await page.waitForLoadState('networkidle');
    ...
    test('query mode selector should be visible', async ({ page }) => {
      ...
      const modeSelector = page.locator('button, [role="combobox"]').filter({ hasText: /Local|Global|Hybrid/i }).first();
  ...
  test.describe('Document Detail Dialog', () => {
    test('documents page should load', async ({ page }) => {
      ...
      const documentsHeader = page.locator('h1, h2').filter({ hasText: /Documents/i }).first();
