...
test.describe('Source Citations Deep Linking', () => {
  ...
  test('should display confidence score based on chunk scores', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="source-citations"]').or(
      page.locator('text=/\\d+\\s+Sources/i')
    )).toBeVisible({ timeout: 30000 });
    
    // The confidence should NOT show "Low (4%)" - it should be higher
    // because we now use chunk scores which are typically 0.5-0.9
    const confidenceText = await page.locator('text=/High|Medium|Low/i').first().textContent();
  ...
  test('document link should use correct URL pattern', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="source-citations"]', { timeout: 30000 }).catch(() => {
      // Alternative selector
      return page.waitForSelector('text=/\\d+\\s+Sources/i', { timeout: 30000 });
    });
    ...
    const navigationPromise = page.waitForURL(/\/documents\/[a-zA-Z0-9-]+/, { timeout: 10000 }).catch(() => null);
    
    // Click on the first document link (ExternalLink button)
    const documentLink = page.locator('[aria-label="Open document in new window"]').first();
  ...
  test('Open Graph Explorer should navigate with entity filter', async ({ page }) => {
    // Submit a query
    const queryInput = page.getByPlaceholder(/ask.*question/i);
    ...
    const exploreButton = page.locator('button:has-text("Open Graph Explorer")');
  ...
  test('document page should support highlight parameter', async ({ page }) => {
    ...
    if (await documentCard.isVisible()) {
      ...
      if (href) {
        ...
        await page.goto(`http://localhost:3000/documents/${docId}?highlight=${encodeURIComponent(highlightText)}`);
        await page.waitForLoadState('networkidle');
  ...
  test('graph page should support entity URL parameters', async ({ page }) => {
    ...
    await page.goto('http://localhost:3000/graph?entities=EDGEQUAKE,LIGHTRAG&focus=EDGEQUAKE');
    await page.waitForLoadState('networkidle');
