...
test.describe('Comprehensive Chat UX Test', () => {
  test('should have perfect chat UX with all features working', async ({ page }) => {
    ...
    const modeSelector = page.locator('select, [role="combobox"], .mode-selector, .select').first();
    ...
    const hasEdgeQuakeContent = pageText?.toLowerCase().includes('edgequake') ||
                               pageText?.toLowerCase().includes('rag') ||
                               pageText?.toLowerCase().includes('retrieval') ||
                               pageText?.toLowerCase().includes('knowledge');
    
    const hasGenericResponse = pageText?.toLowerCase().includes('sorry') ||
    ...
    const cleanText = pageText?.replace(/TFF8NDmVmyfhMoYlcY4VR/g, '')
                              .replace(/__next_/g, '')
                              .replace(/_app_/g, '') || '';
    ...
    for (const pattern of concatenationPatterns) {
      if (typeof pattern === 'string' ? cleanText.includes(pattern) : pattern.test(cleanText)) {
    ...
    const hasProperLayout = await page.locator('.container, .chat-container, .messages, .query-interface').count() > 0;
