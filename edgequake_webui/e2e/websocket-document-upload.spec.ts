...
test.describe('WebSocket Document Upload (OpenAI Tenant)', () => {
  ...
  test('should upload PDF and track status via WebSocket (no polling)', async ({ page }) => {
    ...
    console.log('[Test] Step 3: Verifying optimistic update (immediate appearance)');
    ...
    await expect(statusBadge).toContainText(/Processing|Chunking|Extracting/, { timeout: 10000 });
    ...
    for (let i = 0; i < maxChecks; i++) {
      ...
      if (currentStatus !== lastStatus) {
        ...
        console.log(`[Test] ✓ Status changed #${statusChangeCount}: ${lastStatus} → ${currentStatus}`);
    ...
    const markdownPanel = viewerDialog.locator('[data-testid="markdown-renderer"]');
    ...
    console.log(`[Test] ✓ Markdown content length: ${markdownContent?.length} characters`);

    // Step 13: Final assertions
    console.log('[Test] ✓ All verification steps passed!');
  });

  test('should show real-time updates for multiple concurrent uploads', async ({ page }) => {
    ...
    const firstStatus = documentRows.nth(0).locator('[data-testid="status-badge"]');
    const secondStatus = documentRows.nth(1).locator('[data-testid="status-badge"]');

    // Wait for at least one to start processing
    await expect(firstStatus).toContainText(/Processing|Chunking|Extracting/, { timeout: 10000 });
    console.log('[Test] ✓ First document started processing');

    // Second should also start processing (may be slightly delayed)
    await expect(secondStatus).toContainText(/Processing|Chunking|Extracting|Pending/, { timeout: 10000 });
    console.log('[Test] ✓ Second document status updated');

    console.log('[Test] ✓ Concurrent uploads tracked independently via WebSocket');
  });
});
