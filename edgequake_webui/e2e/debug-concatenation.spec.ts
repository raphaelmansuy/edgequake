...
test.describe('Debug Concatenation Test', () => {
  test('should debug exactly what concatenation issue is found', async ({ page }) => {
    ...
    console.log('Full page text sample (first 1000 chars):', cleanText.substring(0, 1000));
    ...
    const concatenationPatterns = [
      ...
      { name: 'CamelCase regex', pattern: /\b(edge|rag|retrieval|knowledge)[a-z]+[A-Z][a-z]+/i }
    ];

    for (const { name, pattern } of concatenationPatterns) {
      const hasIssue = typeof pattern === 'string' ? cleanText.includes(pattern) : pattern.test(cleanText);
    ...
    for (const pattern of responsePatterns) {
      ...
      if (match) {
        console.log('📝 Found response text:', match[0].substring(0, 200) + '...');
        break;
      }
    }
  });
