...
test.describe('Live Query Test', () => {
  test('should execute a real streaming query and verify response quality', async ({ page }) => {
    ...
    const hasAIContent = pageText?.toLowerCase().includes('artificial intelligence') ||
    ...
    const hasSorryMessage = pageText?.toLowerCase().includes('sorry') ||
    ...
    const responseText = pageText?.replace(/TFF8NDmVmyfhMoYlcY4VR/g, '').replace(/__next_/g, '');
    const hasConcatenationIssue = responseText?.includes('Onceuponatime') ||
                                 ...
                                 /\b(AI|artificial|intelligence|computer|machine|learning)[a-z]+[A-Z][a-z]+/.test(responseText || '');
    ...
    const userMessage = page.locator('div:has-text("' + testQuery + '")').first();
