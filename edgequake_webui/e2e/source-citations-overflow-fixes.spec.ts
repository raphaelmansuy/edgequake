...
test.describe('Source Citations Overflow & Navigation Fixes', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const input = page.getByPlaceholder('Ask questions about your knowledge graph');
  ...
  test('Issue #1: Documents tab chunks should not overflow container', async ({ page }) => {
    // Click Documents tab
    await page.getByRole('tab', { name: /documents/i }).click();
    await page.waitForTimeout(300);
    
    // Get document card container
    const docCard = page.locator('[class*="group"]').filter({ hasText: /Lines \d+-\d+|EdgeQuake|RepoNavigator/i }).first();
  ...
  test('Issue #2: Key Topics should display without overflow', async ({ page }) => {
    // Click Knowledge tab
  ...
  test('Issue #3: Document titles should truncate properly', async ({ page }) => {
    // Click Documents tab
    await page.getByRole('tab', { name: /documents/i }).click();
    await page.waitForTimeout(300);
  ...
  test('Issue #3: Clicking chunk with line numbers navigates correctly', async ({ page }) => {
    // Click Documents tab
    await page.getByRole('tab', { name: /documents/i }).click();
    await page.waitForTimeout(300);
    ...
    if (await lineRangeText.isVisible({ timeout: 2000 })) {
      ...
      if (await highlight.isVisible({ timeout: 3000 })) {
        ...
        console.warn('⚠️  Highlight not visible - line numbers may not be coming from backend');
      }
    } else {
      console.warn('⚠️  Line numbers not displayed - backend not providing line data yet');
...
test.describe('Document Detail Page', () => {
  test('Issue #4: Right sidebar should be scrollable', async ({ page }) => {
    ...
    const firstDoc = page.locator('[class*="cursor-pointer"]').filter({ hasText: /EdgeQuake|Document/i }).first();
    if (await firstDoc.isVisible({ timeout: 5000 })) {
      ...
      if (await sidebar.isVisible({ timeout: 3000 })) {
        ...
        if (await scrollArea.isVisible()) {
          ...
          console.log(`Sidebar scroll: ${scrollHeight}px content in ${clientHeight}px container`);
          
          // If content overflows, verify we can scroll
          if (scrollHeight > clientHeight) {
            // Attempt to scroll
            await scrollArea.evaluate(el => { el.scrollTop = 50; });
  ...
  test('Issue #3 & #4: Line highlighting with stabilo effect', async ({ page }) => {
    ...
    if (await firstDoc.isVisible({ timeout: 5000 })) {
      ...
      if (docId) {
        ...
        await page.goto(`http://localhost:3000/documents/${docId}?start_line=5&end_line=15`);
        ...
        if (count > 0) {
          ...
          const bgImage = await firstHighlight.evaluate(el => 
          ...
          console.warn('⚠️  No line highlights found - content may not support line-based highlighting');
        }
      }
    } else {
...
test.describe('Visual Regression Tests', () => {
  test('Source Citations panel visual snapshot', async ({ page }) => {
    ...
    // Submit query
    await page.getByPlaceholder('Ask questions about your knowledge graph').fill('summarize my knowledge graph');
    await page.getByRole('button', { name: /send/i }).click();
