  ...
  <div className="flex items-start gap-2">
    ...
    <span
      className={`text-[9px] flex-shrink-0 ${
        getConfidenceLabel(chunk.score).color
      }`}
    >
...
const handleDocumentClick = (
  ...
  router.push(
    `/documents/${docId}${params.toString() ? "?" + params.toString() : ""}`
  );
};
```
...
export function ContentRenderer({
  ...
  useEffect(() => {
    if ((startLine === undefined || !contentRef.current) && !highlightText)
      return;
    ...
    const timer = setTimeout(() => {
      ...
      if (startLine !== undefined) {
        ...
        const targetLine = Array.from(lineElements).find(
          (el) =>
            parseInt(el.getAttribute("data-line-number") || "0") >= startLine
        );
        if (targetLine) {
  ...
  return lines
    .map((line, idx) => {
      ...
      if (lineNumber >= startLine && lineNumber <= endLine) {
        return `<mark class="highlight-citation" data-line-number="${lineNumber}">${escapeHtml(
          line
        )}</mark>`;
      }
      return `<span data-line-number="${lineNumber}">${escapeHtml(
        line
      )}</span>`;
    })
    .join("\n");
