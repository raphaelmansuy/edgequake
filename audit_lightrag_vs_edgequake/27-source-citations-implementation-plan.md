      ...
      while ((node = walker.nextNode() as Text)) {
        if (node.textContent?.toLowerCase().includes(searchText)) {
          ...
          if (parent && !parent.classList.contains('citation-highlight')) {
            parent.classList.add('citation-highlight', 'bg-yellow-200', 'dark:bg-yellow-800/50');
            parent.scrollIntoView({ behavior: 'smooth', block: 'center' });
            
            // Remove highlight after 3 seconds
            setTimeout(() => {
              parent.classList.remove('citation-highlight', 'bg-yellow-200', 'dark:bg-yellow-800/50');
            }, 5000);
          }
          break;
        }
