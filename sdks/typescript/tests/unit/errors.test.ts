...
describe("HTTP error classes", () => {
  const errorCases: [string, new (m: string, c?: string) => EdgeQuakeError, number][] = [
...
describe("parseErrorResponse", () => {
  it("maps 400 → BadRequestError", () => {
    const error = parseErrorResponse(400, { code: "BAD_REQUEST", message: "bad" });
  ...
  it("maps 403 → ForbiddenError", () => {
    const error = parseErrorResponse(403, { code: "FORBIDDEN", message: "denied" });
    expect(error).toBeInstanceOf(ForbiddenError);
  });

  it("maps 404 → NotFoundError", () => {
    const error = parseErrorResponse(404, { code: "NOT_FOUND", message: "missing" });
    expect(error).toBeInstanceOf(NotFoundError);
  });

  it("maps 409 → ConflictError", () => {
    const error = parseErrorResponse(409, { code: "CONFLICT", message: "exists" });
    expect(error).toBeInstanceOf(ConflictError);
  });

  it("maps 413 → PayloadTooLargeError", () => {
    const error = parseErrorResponse(413, { code: "TOO_LARGE", message: "big" });
    expect(error).toBeInstanceOf(PayloadTooLargeError);
  });

  it("maps 422 → ValidationError", () => {
    const error = parseErrorResponse(422, { code: "INVALID", message: "bad field" });
    expect(error).toBeInstanceOf(ValidationError);
  });

  it("maps 429 → RateLimitError", () => {
    const error = parseErrorResponse(429, { code: "RATE_LIMIT", message: "slow down" });
    expect(error).toBeInstanceOf(RateLimitError);
  });

  it("maps 500 → InternalError", () => {
    const error = parseErrorResponse(500, { code: "INTERNAL", message: "oops" });
  ...
  it("maps unknown status to EdgeQuakeError", () => {
    const error = parseErrorResponse(418, { code: "TEAPOT", message: "I'm a teapot" });
    expect(error).toBeInstanceOf(EdgeQuakeError);
