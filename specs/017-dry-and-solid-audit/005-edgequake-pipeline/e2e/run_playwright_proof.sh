#!/usr/bin/env bash
# SPEC-017 Playwright visual proof for documents / pipeline UI
set -euo pipefail
...
resolve_backend_url() {
  ...
  # Stale start script port — fall back to env default if healthy there
...
if ! backend_healthy "$API_URL"; then
  echo "→ Starting backend on ${API_URL} (nohup for agent-shell reliability)"
...
(cd "$WEBUI" && E2E_LIVE_STACK=1 EQ_BACKEND_URL="$API_URL" PLAYWRIGHT_BASE_URL="$UI_URL" \
  bunx playwright test e2e/spec017-pipeline-documents.spec.ts e2e/spec017-pipeline-full-ingestion.spec.ts --project=audit --workers=1)

ls -la "$SCREENSHOTS"/*.png 2>/dev/null || true
echo "✓ Playwright pipeline documents proof complete"
