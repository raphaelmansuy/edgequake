#!/usr/bin/env bash
# SPEC-017 Playwright visual proof — starts stack if needed, runs audit specs, writes PNGs.
set -euo pipefail
...
curl -sf --max-time 3 "${API_URL}/health" >/dev/null || {
  echo "✗ Backend not healthy at ${API_URL}"
  exit 1
}

for _ in $(seq 1 30); do
  curl -sf --max-time 3 "$UI_URL/" 2>/dev/null | grep -qi EdgeQuake && break
  sleep 2
done
curl -sf --max-time 3 "$UI_URL/" 2>/dev/null | grep -qi EdgeQuake || {
  echo "✗ EdgeQuake UI not reachable at ${UI_URL}"
  exit 1
}

echo "→ Playwright: UI=${UI_URL} API=${API_URL}"
(cd "$WEBUI" && E2E_LIVE_STACK=1 EQ_BACKEND_URL="$API_URL" PLAYWRIGHT_BASE_URL="$UI_URL" \
  bunx playwright test \
    e2e/spec017-storage-workspace-stats.spec.ts \
    e2e/spec017-storage-conversations.spec.ts \
    --project=audit --workers=1)

ls -la "$SCREENSHOTS"/*.png 2>/dev/null || true
echo "✓ Playwright proof complete"
