...
## Facts Formulation & Agent Verification
...
- **subject**: authoritative source path(s) to check (glob or file path)
- **check**: command(s) or heuristic to run to prove the claim (one-liners encouraged)
- **expected**: expected result or pattern (regex or value)
...
- id: `docs/api.md::F01`
  ...
  subject: `edgequake/crates/edgequake-api/src/routes.rs`
  check: `rg "POST .*?/api/v1/process" -n && rg "status::OK" edgequake/crates/edgequake-api -n`
  expected: `route present && handler returns serde_json::json!({"id": ..})`
...
4. Classify result: `verified|mismatch|not-found|ambiguous` and attach remediation instructions for non-verified facts.
5. For `algorithm` facts, include a code file pointer (path:line-range), a short complexity/behavior summary, and any test/benchmark references.
6. If evidence is ambiguous, escalate by tagging a human reviewer in the craftpad entry and set status `needs-review`.

Minimal evidence entry (scratchpad YAML per fact):
...
facts:
  - id: docs/api.md::F01
    ...
    evidence:
      commands:
        - "rg 'POST .*?/api/v1/process' -n edgequake/crates/edgequake-api/src/routes.rs"
      stdout:
        - "routes.rs:123: router.post('/api/v1/process' ... )"
      excerpt:
        - file: edgequake/crates/edgequake-api/src/routes.rs
          range: 120-130
          snippet: |-
            router.post("/api/v1/process", |req| {
              // handler logic ... returns json!({"id": id})
            });
      notes: "Verified handler registers endpoint and returns id in response"
```
