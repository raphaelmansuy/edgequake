...
**Status:** Approved — Blocked pending `edgequake-pdf2md` v0.2 library features  
**Created:** 2026-02-19  
...
fixes all three: it uses VLM vision as the *primary and only* path, exposes a
...
- [ ] Workspace vision config fields designed (multitenancy types — DB migration plan ready)
- [ ] WebUI settings wireframe for vision provider/model

### Gated on edgequake-pdf2md v0.2

- [ ] Move `edgequake/crates/edgequake-pdf` → `legacy/edgequake-pdf`
- [ ] Remove `crates/edgequake-pdf` from workspace members
- [ ] Pin `edgequake-pdf2md = "0.2"` in workspace deps
- [ ] Update `edgequake-api/Cargo.toml`
- [ ] Update `processor.rs` (tempfile OR `convert_from_bytes` + `ConversionConfig` + `progress_callback`)
- [ ] Update `pipeline_progress_callback.rs` (implement `ConversionProgressCallback`)
- [ ] Add `vision_provider` / `vision_model` to multitenancy types
- [ ] Add `vision_provider` / `vision_model` to API DTOs
- [ ] Add `vision_provider` / `vision_model` to workspace handler
- [ ] Add DB migration (nullable columns, no default)
- [ ] Update Makefile env vars (`EDGEQUAKE_VISION_PROVIDER`, `EDGEQUAKE_VISION_MODEL`)
- [ ] Update `.env.example`
