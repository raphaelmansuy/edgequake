...
Your task is to perform a **deep, end-to-end UX/UI audit** of this web application with **pixel-level attention to layout, scrollable area, containers, borders, spacing, margins, and visual polish**.
...
- Information density balanced with readability – no cramped layouts or wasted space
- Progressive disclosure of complexity – advanced features hidden until needed
...
| Mouse wheel and touchpad scrolling  | Smooth, responsive, no jank                                            |

VERY IMPORTANT: Padding and margin consistency within scrollable areas
...
Create an `audit_ui/` directory example structure:
...
Scroll Area / Fixed Zone is VERY IMPORTANT and must be audited in detail as specified.

You must propose an actionable improvement plan in several markdown files in audit_ui/ that cross references your findings from the audit and the codebase where relevant. Cross all document your are auditing with the relevant code files in the codebase.

- Graph page
- Document page
- Query page 

Are the key pages to focus.
