...
def assess_quality(response: dict, expected_entities: list) -> dict:
    ...
    # Entity bonus
    entity_score = (
        len(entities_found) / len(expected_entities) * 30 if expected_entities else 0
    )
...
def main():
    ...
    for i, test in enumerate(TEST_QUERIES, 1):
        ...
        # Print result
        status = (
            "✓" if quality["score"] >= 50 else "⚠️" if quality["score"] >= 25 else "✗"
        )
        print(
            f"   {status} Response: {response.get('length', 0)} chars, {response.get('sources_count', 0)} sources"
        )
        print(
            f"   {status} Quality: {quality['quality']} (score: {quality['score']:.0f})"
        )
        print(
            f"   {status} Entities: {quality['entities_found']} / {test['expected_entities']}"
        )
