...
Your mission is to improve the usage of the BM25 algorithm in edgequake's search functionality to enhance search relevance and performance.

# Problem Statement

We have observed that the current Rust codebase in edgequake has areas that could benefit from a better bm25 implementation, including tuning of parameters, integration with other ranking signals, and optimization for specific use cases. Improving the BM25 implementation will lead to more accurate search results, better user satisfaction, and enhanced overall system performance.

We have identified tantivy as the primary search engine library used in edgequake, which includes a BM25 implementation. However, there are opportunities to improve how BM25 is configured and utilized within the context of edgequake's specific search requirements.

We envisage that by refining the BM25 implementation, we can achieve using https://crates.io/crates/tantivy better search relevance, improved performance, and a more maintainable codebase. This will make it easier for future developers to work with the search functionality and ensure that edgequake remains competitive in delivering high-quality search experiences. You must assess with brutal honesty if the BM25 implementation can be improved, and if not stop the mission here.
...
- Review https://crates.io/crates/tantivy and its BM25 implementation, see how it can be used in edgequake, define if can really improve search relevance, and performance
...
specs/029-improve-bm25/ooda_loop/
...
- specs/029-improve-bm25/ooda_loop/iteration_01/act.md
...
Each 5 OODA loops you MUST read again your mission at specs/029-improve-bm25/01-improve-bm25.md to ensure you are aligned with the mission objectives
