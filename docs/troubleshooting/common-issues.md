...
### 10. Docker Build Issues

#### Symptom: Frontend build fails with "next-env.d.ts: not found"
...
"/edgequake_webui/next-env.d.ts": not found
```

**Cause**: The `next-env.d.ts` file is auto-generated by Next.js during build and should not be copied explicitly in the Dockerfile.

**Solution**: This has been fixed in the repository. If you encounter this error with an older version, update your `edgequake_webui/Dockerfile` by removing the line:

```dockerfile
COPY edgequake_webui/next-env.d.ts ./
```

The file will be automatically generated during `npx next build`.

**Why**: 
- `next-env.d.ts` is listed in `.gitignore` and never committed
- Next.js regenerates it automatically during build
- It contains TypeScript type references for Next.js
