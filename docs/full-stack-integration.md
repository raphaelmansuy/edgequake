...
async function getNodeDegrees(nodeIds: string[]): Promise<Map<string, number>> {
  ...
  return new Map(data.degrees.map((d) => [d.node_id, d.degree]));
}
...
console.log(
  `Top connected person: ${topPeople[0].label} (${topPeople[0].degree} connections)`
);
```
...
4. _(Previous session)_ `docs/sota-graph-query-comparison.md`
5. _(Previous session)_ `docs/sota-implementation-summary.md`
