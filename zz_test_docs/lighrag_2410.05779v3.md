# **LIGHTRAG: SIMPLE AND RETRIEVAL-AUGMENTED**
...
Beijing University of Posts and Telecommunications University of Hong Kong 2
...
**Second**, these systems often lack the contextual
...
Retrieval-Augmented Generation (RAG) integrates user queries with a collection of pertinent documents sourced from an external knowledge database, incorporating two essential elements: the Retrieval Componentand the Generation Component

for fetching relevant documents or information from the external knowledge database. It identifies and retrieves the most pertinent data based on the input query. 2) After the retrieval process, the generation component takes the retrieved information and generates coherent, contextually relevant responses. It leverages the capabilities of the language model to produce meaningful outputs. Formally, this RAG framework, denoted as*M*, can be defined as follows:

*M*=*G,R*= (*φ, ψ*)*,M*(*q*;*D*

. Ensuring comprehensive

**Rapid Adaptation to New Data**. Enabling

In this framework,*G*and*R*represent the generation module and the retrieval module, respectively, while*q*denotes the input query and *D*refers to the external database. The retrieval module

*R*

2 . 1) The retrieval component is responsible ) =*Gq, ψ*(*q*;*D*ˆ)*,D*ˆ=*φ*(*D*) (1)


---

> Figure 1: Overall architecture of the proposed LightRAG framework. includes two key functionalities: i) Data Indexerφ(·): which involves building a specific data
>
...
*φ*(*·*)must be adept at extracting *D*ˆmust enable rapid and cost-

. Our LightRAG enhances the retrieval
...
the raw text Di. This process effectively reduces the overhead associated with graph operations on Dˆby minimizing the graph's size, leading to more efficient data processing. Our LightRAG offers two advantages through its graph-based text indexing paradigm.

**prehensive Information Understanding**

of global information from multi-hop subgraphs, greatly enhancing LightRAG's ability to handle complex queries that span multiple document chunks. the key-value data structures derived from the graph are optimized for rapid and precise retrieval. This provides a superior alternative to less accurate embedding matching methods (Gao et al., 2023) and inefficient chunk traversal techniques (Edge et al., 2024) commonly used in existing approaches.

**Fast Adaptation to Incremental Knowledge Base**

while ensuring accurate and relevant responses, our LightRAG incrementally updates the knowledge base without the need for complete reprocessing of the entire external database. For a new document *D′*, the incremental update algorithm processes it using the same graph-based indexing steps before, resulting in *D*ˆ*′*= (*V*ˆ*′,E*ˆ*′*). Subsequently, LightRAGcombines the new graph data with the original by taking the union of the node sets Two key objectives guide our approach to fast adaptation for the incremental knowledge base:

**less Integration of New Data**. By applying a consistent methodology to new information, the 

ensuring that historical data remains accessible while enriching the graph without conflicts or redun- dancies.**Reducing Computational Overhead**
...
4 . By combining graph structures with *q*, the retrieval algorithm of LightRAG begins *k*(*l*)and global query keywords *k*(*g*).


---

•(iii)**Incorporating High-Order Relatedness**. To enhance the query with higher-order relatedness, LightRAGfurther gathers neighboring nodes within the local subgraphs of the retrieved graph elements. This process involves the set *{vi|vi∈ V ∧* represent the one-hop neighboring nodes of the retrieved nodes This dual-level retrieval paradigm not only facilitates efficient retrieval of related entities and relations through keyword matching, but also enhances the comprehensiveness of results by integrating relevant structural information from the constructed knowledge graph. 3.3 RETRIEVAL-AUGMENTED ANSWER GENERATION

**Utilization of Retrieved Information** . Utilizing the retrieved information employs a general-purpose LLM to generate answers based on the collected data. This data comprises concatenated values *V*from relevant entities and relations, produced by the profiling function includes names, descriptions of entities and relations, and excerpts from the original text.

**Context Integration and Answer Generation**. By unifying the query with this multi-source text, the LLM generates informative answers tailored to the user's needs, ensuring alignment with the query's intent. This approach streamlines the answer generation process by integrating both context and query into the LLM model, as illustrated in detailed examples (Appendix 7.2). 3.4 COMPLEXITY ANALYSIS OF THELIGHT RAG FRAMEWORK In this section, we analyze the complexity of our proposed LightRAG framework, which can be divided into two main parts. The first part is the graph-based Index phase. During this phase, we use the large language model (LLM) to extract entities and relationships from each chunk of text. As a result, the LLM needs to be called total tokenstimes. Importantly, there is no additional overhead

chunk size

involved in this process, making our approach highly efficient in managing updates to new text. The second part of the process involves the graph-based retrieval phase. For each query, we first utilize the large language model (LLM) to generate relevant keywords. Similar to current Retrieval- Augmented Generation (RAG) systems Gao et al. (2023; 2022); Chan et al. (2024), our retrieval mechanism relies on vector-based search. However, instead of retrieving chunks as in conventional RAG, we concentrate on retrieving entities and relationships. This approach markedly reduces retrieval overhead compared to the community-based traversal method used in GraphRAG.

## **4 E VALUATION**

We conduct empirical evaluations on benchmark data to assess the effectiveness of the proposed LightRAG framework by addressing the following research questions: compare to existing RAG baseline methods in terms of generation performance? dual-level retrieval and graph-based indexing enhance the generation quality of LightRAG? What specific advantages does LightRAG demonstrate through case examples in various scenarios?
...
*ψ*(*q*;*D*ˆ), our LightRAG

P(*·*). It

*•***(RQ1)**: How does LightRAG *•***(RQ2)**: How do
...
particularly pronounced as the dataset size increases. For instance, in the largest dataset (Legal), the disparity widens significantly, with baseline methods achieving only about 20% win rates compared to the dominance of LightRAG. This trend underscores the advantages of graph-enhanced RAG systems in capturing complex semantic dependencies within large-scale corpora, facilitating a more comprehensive understanding of knowledge and leading to improved generalization performance. **Agriculture CS Legal Mix Enhancing Response Diversity with LightRAG** NaiveRAG**LightRAG**NaiveRAG**LightRAG**NaiveRAG : Compared to various baselines, LightRAG demon-

Comprehensiveness 32.4% 67.6% strates a significant advantage in the Diversity metric, particularly within the larger Legal dataset. 38.4% 61.6% 16.4% 83.6% Diversity 23.6% 76.4% Its consistent lead in this area underscores LightRAG's effectiveness in generating a wider range 38.0% 62.0% 13.6% 86.4% Empowerment 32.4% 67.6% 38.8% 61.2% 16.4% 83.6% Overall 32.4% 67.6% of responses, especially in scenarios where diverse content is essential. We attribute this advantage 38.8% 61.2% 15.2% 84.8%

to LightRAG's dual-level retrieval paradigm, which facilitates comprehensive information retrievalRQ-RAG **LightRAG**RQ-RAG **LightRAG**RQ-RAG

Comprehensiveness 31.6% 68.4% from both low-level and high-level dimensions. This approach effectively leverages graph-based text 38.8% 61.2% 15.2% 84.8% Diversity 29.2% 70.8% indexing to consistently capture the full context in response to queries. 39.2% 60.8% 11.6% 88.4% Empowerment 31.6% 68.4% 36.4% 63.6% 15.2% 84.8% Overall 32.4% 67.6% **LightRAG's Superiority over GraphRAG** 38.0% 62.0%: While both LightRAG and GraphRAG use graph-based 14.4% 85.6%

retrieval mechanisms, LightRAG consistently outperforms GraphRAG, particularly in larger datasets HyDE**LightRAG**HyDE**LightRAG**HyDE

Comprehensiveness 26.0% 74.0% with complex language contexts. In the Agriculture, CS, and Legal datasets-each containing millions 41.6% 58.4% 26.8% 73.2% Diversity 24.0% 76.0% of tokens-LightRAG shows a clear advantage, significantly surpassing GraphRAG and highlighting 38.8% 61.2% 20.0% 80.0% Empowerment 25.2% 74.8% 40.8% 59.2% 26.0% 74.0% Overall 24.8% 75.2% its strength in comprehensive information understanding within diverse environments. 41.6% 58.4% 26.4% 73.6%

**Response Variety** GraphRAG**LightRAG** : By integrating low-level retrieval of specific entities with high-level retrieval of GraphRAG**LightRAG**GraphRAG

Comprehensiveness 45.6% 54.4% broader topics, LightRAG boosts response diversity. This dual-level mechanism effectively addresses 48.4% 51.6% 48.4% 51.6% Diversity 22.8% 77.2% both detailed and abstract queries, ensuring a thorough grasp of information. 40.8% 59.2% 26.4% 73.6% Empowerment 41.2% 58.8% **Handling**: This approach is especially valuable in scenarios requiring diverse perspectives. By 45.2% 54.8% 43.6% 56.4% Overall 45.2% 54.8%

48.0% 52.0% 47.2% 52.8%

accessing both specific details and overarching themes, LightRAG adeptly responds to complex queries involving interconnected topics, providing contextually relevant answers. 4.3 A BLATIONSTUDIES(RQ2) We also conduct ablation studies to evaluate the impact of our dual-level retrieval paradigm and the effectiveness of our graph-based text indexing in LightRAG. The results are presented in Table 2.

When handling large

**LightRAG**NaiveRAG**LightRAG**

38.8% 61.2% 32.4% 67.6% 42.8% 57.2% 40.0% 60.0%

**LightRAG**RQ-RAG **LightRAG**

39.2% 60.8% 30.8% 69.2% 42.4% 57.6% 40.0% 60.0%

**LightRAG** HyDE **LightRAG**

40.4% 59.6% 32.4% 67.6% 46.0% 54.0% 42.4% 57.6% **Enhanced LightRAG**GraphRAG**LightRAG**

50.4% 49.6% 36.0% 64.0%**Complex Query**
...
Empowerment 32.4% 67.6% ensures both breadth in the retrieval process and depth in the analysis, providing a comprehensive 38.8% 61.2% Overall 32.4% 67.6% view of the data. As a result, LightRAG achieves balanced performance across multiple dimensions. 38.8% 61.2%

NaiveRAG **-High** NaiveRAG **-High**

Comprehensiveness 34.8% 65.2% **Semantic Graph Excels in RAG** 42.8% 57.2% . We eliminated the use of original text in our retrieval process. Diversity 27.2% 72.8% Surprisingly, the resulting variant, -Origin, does not exhibit significant performance declines across 36.8% 63.2% Empowerment 36.0% 64.0% 42.4% 57.6% Overall 35.2% 64.8% all four datasets. In some cases, this variant even shows improvements ( 44.0% 56.0%

We attribute this phenomenon to the effective extraction of key information during the graph-basedNaiveRAG **-Low** NaiveRAG **-Low**

Comprehensiveness 36.0% 64.0% indexing process, which provides sufficient context for answering queries. Additionally, the original 43.2% 56.8% Diversity 28.0% 72.0% text often contains irrelevant information that can introduce noise in the response. 39.6% 60.4% Empowerment 34.8% 65.2% 42.8% 57.2% Overall 34.8% 65.2%

43.6% 56.4%

4.4 CASESTUDY(RQ3)NaiveRAG **-Origin**NaiveRAG**-Origin**

Comprehensiveness 24.8% 75.2% 39.2% 60.8% Diversity 26.4% 73.6% To provide a clear comparison between baseline methods and our LightRAG, we present specific 44.8% 55.2% Empowerment 32.0% 68.0% case examples in Table 3, which includes responses to a machine learning question from both 43.2% 56.8% Overall 25.6% 74.4%

39.2% 60.8%

the competitive baseline, GraphRAG, and our LightRAG framework. In this instance, LightRAG outperforms in all evaluation dimensions assessed by the LLM judge, including comprehensiveness, diversity, empowerment, and overall quality. Our key observations are as follows:

**i) Comprehensiveness**. Notably, LightRAG covers a broader range of machine learning metrics, showcasing its comprehensiveness and ability to effectively discover relevant information. This highlights the strength of our graph-based indexing paradigm, which excels in precise entity and

NaiveRAG**LightRAG**NaiveRAG**LightRAG**

16.4% 83.6% 38.8% 61.2% 13.6% 86.4% 32.4% 67.6% 16.4% 83.6% 42.8% 57.2% 15.2% 84.8% 40.0% 60.0% NaiveRAG **-High** NaiveRAG **-High**

23.6% 76.4% 40.4% 59.6% 16.8% 83.2% 36.0% 64.0% 22.8% 77.2% 47.6% 52.4% 22.0% 78.0% 42.4% 57.6% *e.g.*in Agriculture and Mix). NaiveRAG **-Low** NaiveRAG **-Low**

19.2% 80.8% 36.0% 64.0% 13.6% 86.4% 33.2% 66.8% 16.4% 83.6% 35.2% 64.8% 18.8% 81.2% 35.2% 64.8% NaiveRAG**-Origin**NaiveRAG**-Origin**

16.4% 83.6% 44.4% 55.6% 14.4% 85.6% 25.6% 74.4% 17.2% 82.8% 45.2% 54.8% 15.6% 84.4% 44.4% 55.6%

relation extraction as well as LLM profiling. LightRAG not only offers a more diverse array of information but also delivers more empowering content. This success is due to LightRAG's hierarchical retrieval paradigm, which combines in-depth
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| recommendation system performs. Below are the most informative metrics: | 1. |  |  |  |  |  |  |  |  |  |  |
...
**LightRAG Key Metrics for Evaluating Movie Recommendation Systems**

involves multiple metrics that assess the effectiveness and accuracy of the recommendations provided to users. Below are some of the most informative metrics commonly used in this context, derived from the data and concepts surrounding recommendation systems. 1.

**Precision and Recall** [...] 3. **Root Mean Squared Error (RMSE) and Mean Squared Error (MSE)**

    4. Area Under the Curve (AUC) [...] 5. F-Measure

comprehensive evaluation of movie recommendation systems employs a range of metrics that capture both explorations of related entities through low-level retrieval to enhance empowerment with broader the accuracy of the recommendations and user satisfaction. By effectively leveraging these metrics, system explorations via high-level retrieval to improve answer diversity. Together, these approaches capture adesigners can optimize recommendation engines to deliver more personalized and relevant experiences.

**LLM DecisionComprehensiveness** comprehensive global perspective of the knowledge domain, contributing to better RAG performance. : "Winner": "Answer 2 (LightRAG)",

"Explanation": "Answer 2 provides a broader array of metrics including MAPK, AUC, and user engagement metrics alongside precision, recall, RMSE, and F-Measure. This gives a fuller view of how recommendation

4.5 M ODELCOST ANDADAPTABILITY systems can be evaluated. Answer 1, while detailed, is more focused on traditional metrics and lacks the inclusion of engagement metrics which are crucial for understanding user interaction." We compare the cost of our LightRAG with that of the top-performing baseline, GraphRAG, from two key Diversity : "Winner": "Answer 2 (LightRAG)", perspectives. First, we examine the number of tokens "Explanation": "Answer 2 not only covers a wide variety of metrics but also includes nuanced explanations of how some metrics interrelate and differ from one another, like the inclusion of both RMSE and MSE, as and API calls during the indexing and retrieval prowell as the details behind AUC. In contrast, Answer 1 sticks primarily to standard metrics without much cesses. Second, we analyze these metrics in relation exploration of potential nuances." to handling data changes in dynamic environments.

**Empowerment**: "Winner": "Answer 2 (LightRAG)", The results of this evaluation on the legal dataset are "Explanation": "Answer 2 empowers the reader more effectively by detailing how each metric functions and presented in Table 2. In this context, its importance in evaluating recommendation systems. By providing context such as the trade-offs between sents the token overhead for entity and relationship extraction, precision and recall and emphasizing user engagement metrics, it enables readers to make more informed judgments and understand the implications of different metrics. Answer 1 is more straightforward but lacks of tokens allowed per API call, and the depth of insight regarding why these metrics matter." In the retrieval phase, GraphRAG generates 1,399 communities, with 610 level-2 communities **Overall Winner** : "Winner": "Answer 2 (LightRAG)", actively utilized for retrieval in this experiment. Each community report averages 1,000 tokens,"Explanation": "While Answer 1 is more direct and systematic, Answer 2 excels in comprehensiveness, resulting in a total token consumption of 610,000 tokens (610 communities diversity, and empowerment. It provides a richer exploration of the topic, including insights into user community). Additionally, GraphRAG's requirement to traverse each community individually leads engagement and nuanced differences between metrics. This depth and breadth make it more informative for readers seeking to thoroughly understand the evaluation of movie recommendation systems."

: Evaluating the effectiveness of movie

**Precision** [...] 2. **Recall**[...]

[...] 5. **Root Mean Squared Error (RMSE)** [...] 6. : In summary, metrics such as precision, recall, F1 score, MAP,

: Evaluating movie recommendation systems

**Mean Average Precision at K (MAPK)** [...] 2.

[...]

[...] 6. **User Engagement Metrics** [...] **Conclusion** : A

ANALYSIS(RQ4)

> Figure 2: Comparison of Cost in Terms of Tokens and API Calls for GraphRAG and Ligh-
>

tRAG on the Legal Dataset.

Phase Retrieval Phase Incremental Text Update Model GraphRAG Ours GraphRAG Ours
...
*T*extractrepre- Calls max

*C*maxdenotes the maximum number

*C*extractindicates the number of API calls required for extraction.

to hundreds of API calls, significantly increasing retrieval overhead. In contrast, LightRAG optimizes this process by using fewer than 100 tokens for keyword generation and retrieval, requiring only a single API call for the entire process. This efficiency is achieved through our retrieval mechanism,
...
William Brannon, Suyash Fulay, Hang Jiang, Wonjune Kang, Brandon Roy, Jad Kabbara, and Deb Roy. Congrat: Self-supervised contrastive pretraining for joint graph and text embeddings. *preprint arXiv:2305.14321* , 2023. Chi-Min Chan, Chunpu Xu, Ruibin Yuan, Hongyin Luo, Wei Xue, Yike Guo, and Jie Fu. Rq-rag: Learning to refine queries for retrieval augmented generation. 2024. Runjin Chen, Tong Zhao, AJAY KUMAR JAISWAL, Neil Shah, and Zhangyang Wang. Llaga: Large language and graph assistant. In *International Conference on Machine Learning (ICML)* Darren Edge, Ha Trinh, Newman Cheng, Joshua Bradley, Alex Chao, Apurva Mody, Steven Truitt, and Jonathan Larson. From local to global: A graph rag approach to query-focused summarization. *arXiv preprint arXiv:2404.16130* , 2024. Shahul Es, Jithin James, Luis Espinosa Anke, and Steven Schockaert. Ragas: Automated evaluation of retrieval augmented generation. In *International Conference of the European Chapter of the Association for Computational Linguistics (EACL)* Wenqi Fan, Yujuan Ding, Liangbo Ning, Shijie Wang, Hengyun Li, Dawei Yin, Tat-Seng Chua, and Qing Li. A survey on rag meeting llms: Towards retrieval-augmented large language models. In *International Conference on Knowledge Discovery and Data Mining (KDD)* Luyu Gao, Xueguang Ma, Jimmy Lin, and Jamie Callan. Precise zero-shot dense retrieval without relevance labels. *arXiv preprint arXiv:2212.10496* Yunfan Gao, Yun Xiong, Xinyu Gao, Kangxiang Jia, Jinliu Pan, Yuxi Bi, Yi Dai, Jiawei Sun, and Haofen Wang. Retrieval-augmented generation for large language models: A survey. *preprint arXiv:2312.10997* , 2023. Yichuan Li, Kaize Ding, and Kyumin Lee. Grenade: Graph-centric language model for self- supervised representation learning on text-attributed graphs. In *Empirical Methods in Natural Language Processing (EMNLP)* Hao Liu, Jiarui Feng, Lecheng Kong, Ningyue Liang, Dacheng Tao, Yixin Chen, and Muhan Zhang. One for all: Towards training one graph model for all classification tasks. In *Conference on Learning Representations (ICLR)* Yuanjie Lyu, Zhiyu Li, Simin Niu, Feiyu Xiong, Bo Tang, Wenjin Wang, Hao Wu, Huanyong Liu, Tong Xu, and Enhong Chen. Crud-rag: A comprehensive chinese benchmark for retrieval- augmented generation of large language models. Hongjin Qian, Peitian Zhang, Zheng Liu, Kelong Mao, and Zhicheng Dou. Memorag: Moving towards next-gen rag via memory-inspired knowledge discovery, 2024. URL `org/abs/2409.05591`. Ori Ram, Yoav Levine, Itay Dalmedigos, Dor Muhlgay, Amnon Shashua, Kevin Leyton-Brown, and Yoav Shoham. In-context retrieval-augmented language models. *for Computational Linguistics (TACL)* , 11:1316-1331, 2023. 

*Conference on Neural Information Processing Systems (NeurIPS)* Alireza Salemi and Hamed Zamani. Evaluating retrieval quality in retrieval-augmented generation. In*ACM International Conference on Research and Development in Information Retrieval (SIGIR)* pp. 2395-2400, 2024.
...
*International Conference on* , pp. 2745-2757, 2023.
...
Viju Sudhi, Sinchana Ramakanth Bhat, Max Rudat, and Roman Teucher. Rag-ex: A generic framework for explaining retrieval augmented generation. In *Research and Development in Information Retrieval (SIGIR)*

*ACM International Conference on* , pp. 2776-2780, 2024.
...
generated knowledge graph, targeting relevant entities and relationships. The retrieved information is organized into three components: entities, relationships, and corresponding text chunks. This structured data is then fed into the LLM, enabling it to generate a comprehensive answer to the query. 7.3 O VERVIEW OF THE PROMPTS USED IN LIGHTRAG 7.3.1 PROMPTS FOR GRAPHGENERATION

> Figure 4: Prompts for Graph Generation
>

The graph construction prompt outlined in Figure 4 is designed to extract and structure entityrelationship information from a text document based on specified entity types. The process begins by identifying entities and categorizing them into types such as organization, person, location, and event. It then provides detailed descriptions of their attributes and activities. Next, the prompt identifies relationships between these entities, offering explanations, assigning strength scores, and summarizing the relationships using high-level keywords. 7.3.2 PROMPTS FOR QUERYGENERATION
...
Indigenous perspectives shape corporate mergers by emphasizing communal ownership and collaborative partnerships. Corporations must adopt respectful, inclusive practices that honor Indigenous cultural values and rights, paving the way for sustainable, mutually beneficial outcomes. LLM must identify which answer performs better and provide a rationale for its choice. Ultimately,

**LLM DecisionComprehensiveness** an overall winner is determined based on performance across all three dimensions, accompanied by a : "Winner": "Answer 2 (LightRAG)",

"Explanation": "Answer 2 provides a thorough exploration of the indigenous perspectives by discussing detailed summary that justifies the decision. The evaluation is structured in JSON format, ensuringcultural significance, collaboration, and legal frameworks, including specific examples and detailed insights. clarity and consistency, and facilitating a systematic comparison between the two answers. In contrast, Answer 1, while informative, lacks the same depth in analyzing the various dimensions of the indigenous perspectives on ownership and collaboration." 7.4 CASESTUDY: COMPARISON BETWEEN LIGHTRAG

**Diversity** : "Winner": "Answer 2 (LightRAG)", "Explanation": "Answer 2 presents a wider array of perspectives, focusing on the communal aspect of To further illustrate LightRAG's superiority over baseline models in terms of comprehensiveness, land ownership, spiritual connections, and practical examples of collaboration. It covers more ground by empowerment, and diversity, we present a case study comparing LightRAG and NaiveRAG incontrasting Indigenous views with Western notions, whereas Answer 1 primarily focuses on corporate

> Table 5. This study addresses a question regarding indigenous perspectives in the context of corporate strategies and outcomes with limited perspective." mergers. Notably, LightRAG offers a more in-depth exploration of key themes related to indigenous Empowerment: "Winner": "Answer 2 (LightRAG)",
>

perspectives, such as cultural significance, collaboration, and legal frameworks, supported by specific"Explanation": "Answer 2 equips the reader with nuanced understanding and actionable insights by and illustrative examples. In contrast, while NaiveRAG provides informative responses, it lacks highlighting the importance of collaboration and the legal frameworks involved. It empowers corporations the depth needed to thoroughly examine the various dimensions of indigenous ownership and by illustrating how adopting an inclusive approach can facilitate better outcomes. Answer 1, while practical, does not emphasize the moral or ethical implications as strongly." collaboration. The dual-level retrieval process employed by LightRAG enables a more comprehensive investigation of specific entities and their interrelationships, facilitating extensive searches that **Overall Winner** : "Winner": "Answer 2 (LightRAG)",

AND THE BASELINENAIVERAG.

effectively capture overarching themes and complexities within the topic. "Explanation": "Answer 2 excels overall due to its comprehensive exploration, diversity of perspectives, and empowerment of the reader with actionable insights about indigenous perspectives and collaboration in corporate mergers. Although Answer 1 is more direct, the depth and breadth of Answer 2 make it the stronger response."
