...
test.describe("Scroll Fixes Verification", () => {
  test("Entity Browser scrolls with mouse wheel", async ({ page }) => {
    ...
    const scrollArea = page.locator(
      '[aria-label="Entity browser"] [data-radix-scroll-area-viewport]'
    );
  ...
  test("Details Panel scrolls when content overflows", async ({ page }) => {
    ...
    const scrollArea = page.locator(
      'aside:has-text("Details") [data-radix-scroll-area-viewport]'
    );
  ...
  test("Mobile Legend does not overlay graph", async ({ page }) => {
    ...
    const floatingLegend = page.locator(
      '.absolute.bottom-4.right-4 [role="region"]'
    );
...
_Implementation plan created December 30, 2025_  
_Based on Playwright browser automation verification_
