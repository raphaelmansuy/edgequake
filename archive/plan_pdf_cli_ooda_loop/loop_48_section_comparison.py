...
def extract_sections(filepath, name):
    ...
    # Find key sections
    sections["abstract"] = extract_between_markers(
        lines, ["Abstract"], ["Introduction", "1."]
    )
    sections["introduction"] = extract_between_markers(
        lines, ["Introduction", "1."], ["Related", "2."]
    )
    sections["related_work"] = extract_between_markers(
        lines, ["Related", "2."], ["3.", "Method"]
    )
    sections["method"] = extract_between_markers(
        lines, ["Method", "3."], ["Result", "Experiment", "4."]
    )
    sections["results"] = extract_between_markers(
        lines, ["Result", "Experiment", "4."], ["Conclusion", "5."]
    )
    sections["conclusion"] = extract_between_markers(
        lines, ["Conclusion", "5."], ["Reference", "Acknowledg"]
    )
    sections["references"] = extract_between_markers(
        lines, ["Reference", "Acknowledg"], ["999999999"]
    )
...
def main():
    ...
    for section_name in section_names:
        ...
        result = analyze_section(
            gold_sec, current_sec, section_name.replace("_", " ").title()
        )
        print(result)
