...
test("first-time user sees onboarding and creates workspace", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: /welcome to edgequake/i })
  ).toBeVisible();
...
test("returning user automatically enters last workspace", async ({
  ...
  await expect(page.getByTestId("workspace-selector")).toHaveText(
    "My Workspace"
  );
...
```typescript
test("metadata sidebar shows lineage and allows exploration", async ({
  page,
}) => {
...
```typescript
test("document detail page loads within performance budget", async ({
  page,
}) => {
...
test("document detail page matches visual snapshot", async ({ page }) => {
  ...
  await expect(page.getByTestId("lineage-tree")).toHaveScreenshot(
    "lineage-tree.png"
  );
});
